...
#ifdef GPU_ACC
		if (get_acc_solve()){
			checkGPU(gpuMemcpy(ScalePermstruct->d_R, ScalePermstruct->R, sizeof(float) * (size_t)m, gpuMemcpyHostToDevice));
			checkGPU(gpuMemcpy(ScalePermstruct->d_C, ScalePermstruct->C, sizeof(float) * (size_t)n, gpuMemcpyHostToDevice));
		}
#endif
...
// if ( 0){
		...
		------------------------------------------------------------*/
		ldx = ldb;
		checkGPU(gpuMalloc((void**)&X, ((size_t)ldx) * nrhs * sizeof(float)));
		sscale_and_copy_rhs_wrap(B, ldb, X, ldx, m_loc, nrhs, fst_row, notran,rowequ, colequ,ScalePermstruct);
	...
	// if ( 0){
#ifdef GPU_ACC
		psPermute_Dense_Matrix_gpu_wrap(fst_row, m_loc,n,
				X, ldx, B, ldb, nrhs,grid,SOLVEstruct);
		sundo_equilibration_rhs_wrap(B, ldb, m_loc, nrhs, fst_row, notran,rowequ, colequ,ScalePermstruct);
		checkGPU(gpuFree(X));
