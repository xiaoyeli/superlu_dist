...
/*! @file
 * \brief GMRES inner-solve kernels used by iterative refinement (psgsrfs /
 ...
 * LU factors, M^{-1} = U^{-1} L^{-1} (applied by psgstrs / psgstrs3d):
 *
 *     A M^{-1} u = r,      d = M^{-1} u.
 *
 ...
 * iterative-refinement loop in psgsrfs/psgsrfs3d is unchanged: it still forms
 * the true residual r = b - A x and tests the backward error for convergence.
 *
 * psgmres()   : 2D-grid kernel (uses psgsmv + psgstrs).
 * psgmres3d() : 3D-grid kernel.  All Krylov work is on z-layer 0; each
 *               preconditioner apply is the collective 3D solve
 *               (psgstrs3d[_newsolve]); the residual norm is broadcast down the
 *               z-axis so every layer performs the same number of collective
 *               solves (same scheme as psgsrfs3d).
 * </pre>
 */
 ...
 * superlu_s{scal,axpy,gemv,trsv} wrappers (i.e. the vendor/OpenBLAS BLAS),
 ...
 * reason local_strtri exists in psgstrs.c.  The GMRES dense work is tiny, so
 * plain loops cost nothing and keep the solver portable.
 * ---------------------------------------------------------------------- */

/*! \brief x[0:n] *= a */
static void gm_scal(int_t n, float a, float *x)
{ int i; for (i = 0; i < n; ++i) x[i] *= a; }

/*! \brief y[0:n] += a * x[0:n] */
static void gm_axpy(int_t n, float a, const float *x, float *y)
...
static void gm_gemv(char op, int_t m, int k, float alpha, const float *V,
...
static void gm_trsvU(int k, float *U, int ldh, float *b)
{
    int i, j;
...
static float
psgmres_dot(int_t n_loc, const float *x, const float *y, gridinfo_t *grid)
{
    int i;
    float local = 0.0, global;
    for (i = 0; i < n_loc; ++i) local += x[i] * y[i];
    
    MPI_Allreduce(&local, &global, 1, MPI_FLOAT, MPI_SUM, grid->comm);
    return global;
}

/*! \brief Global 2-norm of a length-n_loc local vector. */
static float
psgmres_norm2(int_t n_loc, const float *x, gridinfo_t *grid)
{
    return sqrt(psgmres_dot(n_loc, x, x, grid));
}

/*! \brief
 * <pre>
 * PSGMRES (2D inner kernel).  Solve  A d = r  for the correction d, using
 * RIGHT-preconditioned restarted GMRES with M^{-1} = U^{-1} L^{-1} (psgstrs).
 *
 * On entry  X[0..m_loc-1] holds the right-hand side r.
 * On exit   X[0..m_loc-1] holds the correction d (initial guess d = 0).
 *
 * gsmv_comm   must be initialized (A in the psgsmv-permuted state).
 * SOLVEstruct must be configured for a single RHS (as psgsrfs uses it).
 * *totit (if non-NULL) is incremented by the number of GMRES iterations.
 * </pre>
 */
void
psgmres(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
        sLUstruct_t *LUstruct, sScalePermstruct_t *ScalePermstruct,
        ...
        sSOLVEstruct_t *SOLVEstruct, int *totit,
        SuperLUStat_t *stat, int *info)
{
    int     iam = grid->iam, i, k, it, ldh, nrit, conv, total = 0;
    float gamma, tmpd, scal_factor;
    float  rho, rho0, delta;;
    float *work, *V, *hess, *givens_c, *givens_s, *bb, *rhs, *z, *Ax;
    float zero = 0.0, one = 1.0, neg_one = -1.0;
...

    {
        ...
        if ( !(work = floatMalloc_dist(lwork)) )
            ABORT("Malloc fails for work[] in psgmres()");
    }
    givens_c = work;
    ...
    while ( !conv ) {
        ...
        if ( total > 0 ) {
            psgsmv(0, A, grid, gsmv_comm, X, Ax);
            for (i = 0; i < m_loc; ++i) V[i] = rhs[i] - Ax[i];
        } else {
            for (i = 0; i < m_loc; ++i) V[i] = rhs[i];
        }

        rho = psgmres_norm2(m_loc, V, grid);
        if ( total == 0 ) rho0 = rho;
	...
	scal_factor = 1.0/rho;
        ...
        for (it = 0; it < restart; ++it) {
            float *w   = &V[(size_t)(it+1)*m_loc];
            float *vit = &V[(size_t)it*m_loc];
            ...
            psgstrs(options, n, LUstruct, ScalePermstruct, grid, z,
                    m_loc, fst_row, m_loc, 1, SOLVEstruct, stat, info);
            psgsmv(0, A, grid, gsmv_comm, z, w);
...
static void
psgmres3d_prec(superlu_dist_options_t *options, int_t n, sLUstruct_t *LUstruct,
               sScalePermstruct_t *ScalePermstruct,
               strf3Dpartition_t *trf3Dpartition, gridinfo3d_t *grid3d,
               float *v, int_t m_loc, int_t fst_row,
               sSOLVEstruct_t *SOLVEstruct, SuperLUStat_t *stat, int *info)
{
    if ( get_new3dsolve() )
        psgstrs3d_newsolve(options, n, LUstruct, ScalePermstruct, trf3Dpartition,
                           grid3d, v, m_loc, fst_row, m_loc, 1, SOLVEstruct,
                           stat, info);
    else
        psgstrs3d(options, n, LUstruct, ScalePermstruct, trf3Dpartition,
                  grid3d, v, m_loc, fst_row, m_loc, 1, SOLVEstruct, stat, info);
}

/*! \brief
 * <pre>
 * PSGMRES3D (3D inner kernel).  Solve  A d = r  for the correction d, using
 * RIGHT-preconditioned restarted GMRES with M^{-1} = U^{-1}L^{-1}
 * (psgstrs3d[_newsolve]), for the 3D solve layout.
 *
 ...
 */
void
psgmres3d(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
          sLUstruct_t *LUstruct, sScalePermstruct_t *ScalePermstruct,
          gridinfo3d_t *grid3d, strf3Dpartition_t *trf3Dpartition,
          psgsmv_comm_t *gsmv_comm, float *X, int_t m_loc, int_t fst_row,
          int restart, int maxit, double rtol, double atol, int gs,
          sSOLVEstruct_t *SOLVEstruct, int *totit,
          SuperLUStat_t *stat, int *info)
{
    ...
    float gamma, tmpd, scal_factor;
    float rho, rho0, delta, rbuf[2];
    float *work = NULL, *V = NULL, *hess = NULL, *givens_c = NULL,
    ...
    if ( !(pc = floatMalloc_dist(SUPERLU_MAX(m_loc,1))) )
        ABORT("Malloc fails for pc[] in psgmres3d()");
    if ( layer0 ) {
        size_t lwork = (size_t)2*restart + (restart+1)
                     + (size_t)(restart+1)*restart
                     + (size_t)m_loc*(restart+1) + 2*(size_t)m_loc;
        if ( !(work = floatMalloc_dist(lwork)) )
            ABORT("Malloc fails for work[] in psgmres3d()");
        givens_c = work;
        givens_s = givens_c + restart;
        bb       = givens_s + restart;
        hess     = bb + (restart+1);
	...
	if ( layer0 ) {
        ...
        MPI_Bcast(rbuf, 2, MPI_FLOAT, 0, grid3d->zscp.comm);
        ...
        for (it = 0; it < restart; ++it) {
            float *w   = layer0 ? &V[(size_t)(it+1)*m_loc] : NULL;
            float *vit = layer0 ? &V[(size_t)it*m_loc]     : NULL;
            ...
            psgmres3d_prec(options, n, LUstruct, ScalePermstruct, trf3Dpartition,
                           grid3d, pc, m_loc, fst_row, SOLVEstruct, stat, info);

            if ( layer0 ) {
                psgsmv(0, A, grid, gsmv_comm, pc, w);
            ...
            MPI_Bcast(&rho, 1, MPI_FLOAT, 0, grid3d->zscp.comm);
            if ( rho < atol || rho/rho0 < rtol || total >= maxit ) {
