...
/***** YL: remove the allocation in the following as sGatherNRformat_loc3d_allgrid instead of zGatherNRformat_loc3d has been called, which already allocate A->Store on all grids
...
*/
    ...
    remainder =  Astore->nnz_loc%chunk;
        ...
        if(i==Nchunk-1){
            ...
            MPI_Bcast( &(Astore->nzval[idx]),  count*sizeof(float), MPI_BYTE, 0, grid3d->zscp.comm);
