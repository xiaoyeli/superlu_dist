...
//////
	...
	if ((nrhs > 0) && (*info == 0))
	{
		...
		(!Solve3D || !get_new3dsolve() || !get_new3dsolvetreecomm() ||
		 options->IterRefine)) {
		ABORT("GPURES psgssvx3d path currently assumes Solve3D, NEW3DSOLVE=1, NEW3DSOLVETREECOMM=1, and NOREFINE.");
	    }
		...
		if (options->GPURES == YES) {
		    ...
		    checkGPU(gpuMalloc((void**)&X,
				       sizeof(float) * (size_t)ldx * (size_t)nrhs));
		    sscale_and_copy_rhs_wrap(B, ldb, X, ldx, m_loc, nrhs,
					     fst_row, notran, rowequ, colequ,
					     ScalePermstruct);
#else
		    ABORT("GPURES requires GPU_ACC in psgssvx3d().");
#endif
		} else {
		/* ------------------------------------------------------
	...
	if (grid3d->zscp.Iam == 0)  /* on 2D grid-0 */
	{
		...
		psPermute_Dense_Matrix_gpu_wrap(fst_row, m_loc, n,
						 X, ldx, B, ldb, nrhs,
						 grid, SOLVEstruct);
		sundo_equilibration_rhs_wrap(B, ldb, m_loc, nrhs, fst_row,
