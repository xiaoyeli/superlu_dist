...
void psgssvx3d_d2(superlu_dist_options_t *options, SuperMatrix *A,
...
{
    /* ---- double-precision IR helpers (see psgsmv_d2.c, psgsrfs_d2.c) ---- */
    extern void psgsmv_init_fp64(SuperMatrix *, int_t *, gridinfo_t *,
				 psgsmv_comm_t *);
    extern void psgsrfs_d2(superlu_dist_options_t *, int, SuperMatrix *, float,
			   sLUstruct_t *, sScalePermstruct_t *, gridinfo_t *,
			   float *, int_t, float *, int_t, int, sSOLVEstruct_t *,
			   float *, SuperLUStat_t *, int *, double *);
...
//////
	...
	if(Solve3D){
		...
		if ( options->SolveInitialized == YES ) { /* Initialized before */
			...
			if (get_acc_solve()) psgstrs_delete_device_lsum_x(SOLVEstruct);
		}
	    }
	...
	if ((nrhs > 0) && (*info == 0))
	{
		...
		if (options->IterRefine)
		{
			...
			*/
		    ...
		    psgsrfs3d_d2 (options, n, A, anorm, LUstruct, ScalePermstruct, grid3d, trf3Dpartition,
				B, ldb, X, ldx, nrhs, SOLVEstruct1, err_bounds, stat, info, xtrue);

		    /* Return the componentwise backward error in berr[] too. */
		    for (int jj = 0; jj < nrhs; ++jj) berr[jj] = err_bounds[jj + 2*nrhs];
		...
		if (options->IterRefine)
		{
			...
			*/
		    ...
		    psgsrfs_d2 (options, n, A, anorm, LUstruct, ScalePermstruct, grid,
				B, ldb, X, ldx, nrhs, SOLVEstruct1, err_bounds, stat, info, xtrue);
