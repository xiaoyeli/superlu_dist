...
)
{
	...
	double temp,temp1;
        #ifdef HAVE_SYCL
        sycl::group wrk_grp = item.get_group();
        using tile_t = double[MAXSUPER];
        tile_t& temp2 = *sycl::ext::oneapi::group_local_memory_for_overwrite<tile_t>(wrk_grp);
        #else
...
//  int_t lk,k,knsupc;
 ...
 sycl_get_queue()->parallel_for<class dlsum_fmod_inv_gpu_mrhs_kernel>
   (sycl::nd_range<3>(global, dimBlock), [=] (sycl::nd_item<3> item)  {
     dlsum_fmod_inv_gpu_mrhs(nbcol_loc,nblock_ex,lsum,x,nrhs,maxsup,nsupers,fmod,LBtree_ptr,LRtree_ptr,ilsum,Lrowind_bc_dat,Lrowind_bc_offset,Lnzval_bc_dat,Lnzval_bc_offset,Linv_bc_dat,Linv_bc_offset,Lindval_loc_bc_dat,Lindval_loc_bc_offset, xsup,grid,recvbuf_BC_gpu,recvbuf_RD_gpu,maxrecvsz, item);
   });
#else
...
/*          @ingroup magma_kernel */
/*  *******************************************************************************\/ */
/* __device__ void magma_sum_reduce(int n, int i, double *x) */
/* { */
/*   __syncthreads(); */
/*   if ( n > 1024 ) { if ( i < 1024 && i + 1024 < n ) { x[i] += x[i+1024]; }  __syncthreads(); } */
/*   if ( n >  512 ) { if ( i <  512 && i +  512 < n ) { x[i] += x[i+ 512]; }  __syncthreads(); } */
...
/* /\******************************************************************************\/ */
/* static __device__ void */
/* gemv_device_dlsum_fmod(int_t m, int_t n, double alpha, */
/*                        const double *__restrict__ A, int_t lda, */
/*                        const double *__restrict__ x, int_t incx, double beta, */
/*                        double *__restrict__ y, int_t incy */
/* #ifdef HAVE_SYCL */
...
/* 	    #ifdef HAVE_SYCL */
/*             temp = sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>(lsum[il+irow + j*iknsupc]).fetch_add( -temp1 ); */
/* 	    #else */
/* 	    temp=atomicAdd(&lsum[il+irow + j*iknsupc],-temp1); */
/* 	    #endif */
/*           } */
...
/*           if (i == nbrow + lsub[lptr1_tmp + 1] - 1) { */
/* 	    #ifdef HAVE_SYCL */
/*             fmod_tmp = sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>( fmod[lk*aln_i] ).fetch_sub( 1 ); */
/* 	    #else */
/* 	    fmod_tmp=atomicSub(&fmod[lk*aln_i],1); */
/* 	    #endif */
/*             // __threadfence(); */
...
/*                     double &regC = rC[ni][mi]; */
/* #ifdef HAVE_SYCL */
/*                     temp = sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>( lsum[il+irow + coord_dCn*iknsupc] ).fetch_add( -regC ); */
/* #else */
/* 		    temp=atomicAdd(&lsum[il+irow + coord_dCn*iknsupc],-regC); */
/* #endif */
/*                   } */
...
/*           if (tid == 0) { */
/* #ifdef HAVE_SYCL */
/*             temp = sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>( fmod[lk*aln_i] ).fetch_sub( 1 ); */
/* #else */
/* 	    fmod_tmp=atomicSub(&fmod[lk*aln_i],1); */
/* #endif */
/* 	  } */
...
/*         for (i = 0; i < knsupc; ++i) { */
/* #ifdef HAVE_SYCL */
/*           temp = sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>( lsum[il+i + j*knsupc] ).fetch_add( recvbuf_RD_gpu[maxrecvsz*lib*2+ii*maxrecvsz + i + j*knsupc] ); */
/* #else */
/* 	  temp=atomicAdd(&lsum[il+i + j*knsupc], recvbuf_RD_gpu[maxrecvsz*lib*2+ii*maxrecvsz + i + j*knsupc]  ); */
/* #endif */
/* 	} */
/*       } */
/* #ifdef HAVE_SYCL */
/*       fmod_tmp = sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>( fmod[lib*aln_i] ).fetch_sub( 1 ); */
/* #else */
/*       fmod_tmp=atomicSub(&fmod[lib*aln_i],1); */
/* #endif */
/*     } */
...
/*           } */
/*           #ifdef HAVE_SYCL */
/*           temp = sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>(lsum[il+offset]).fetch_add(-temp1); */
/*           #else */
/*           temp=atomicAdd(&lsum[il+offset],-temp1); */
/*           #endif */
/*         }else{ */
...
/*             } */
/*             #ifdef HAVE_SYCL */
/*             temp = sycl::atomic_ref<double, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>(lsum[il+offset + j*iknsupc]).fetch_add( -temp1 ); */
/*             #else */
/*             temp=atomicAdd(&lsum[il+offset + j*iknsupc],-temp1); */
/*             #endif */
/*           } */
...
/*         ik = lloc[ub]; */
/*         #ifdef HAVE_SYCL */
/*         bmod_tmp = sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space>(bmod[ik*aln_i]).fetch_sub( 1 ); */
/*         #else */
/*         bmod_tmp=atomicSub(&bmod[ik*aln_i],1); */
/*         #endif */
/*         // printf("ik %5d bmod[ik*aln_i] %5d\n",ik,bmod[ik*aln_i]); */
