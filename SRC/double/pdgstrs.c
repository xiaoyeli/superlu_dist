 ...
 * LAPACK dtrtri_ dispatches AVX-512 kernels that raise SIGILL (illegal
 * instruction) on this node's AMD EPYC 7763 CPU (AVX2 only).  Verified: with
 * the stock dtrtri_ the GPU-solve "computing inverse of diagonal blocks" step
 ...
 * to the standard LAPACK dtrtri_ and this inline routine can be removed.
 *
 * Inline triangular matrix inversion (drop-in replacement for LAPACK dtrtri_).
 * For the small diagonal supernode blocks factored here (n <= SUPERLU_MAXSUP)
 * an O(n^3) loop is perfectly acceptable.
 * ===========================================================================
 *
 ...
 */
static void local_dtrtri(const char *uplo, const char *diag,
                         int *pn, double *A, int *plda, int *info)
{
    int n = *pn, lda = *plda;
    int i, j, k;
    ...
    *info = 0;
    ...
    if (uplo[0] == 'L' || uplo[0] == 'l') {
        ...
        for (j = 0; j < n; j++) {
            if (!unit) {
                if (A[j*lda+j] == 0.0) { *info = j+1; return; }
        ...
        for (j = n-1; j >= 0; j--) {
            ...
            for (i = j-1; i >= 0; i--) {
                ...
                double s = A[j*lda+i] * (unit ? 1.0 : A[j*lda+j]); /* U(i,j)*U^{-1}(j,j) */
                for (k = i+1; k < j; k++)
     ...
     *---------------------------------------------------*/
	 ...
	 if ( myrow == krow ) {
	     ...
	     if ( mycol == kcol ) { /* diagonal process */
		   ...
		   * local_dtrtri avoids SIGILL from this cluster's AVX-512-only
		   * OpenBLAS on the AVX2 EPYC CPU.  On a normal BLAS install, use
		   * the standard LAPACK dtrtri_ instead (see note at its definition). */
   		  local_dtrtri("L","U",&knsupc,Linv,&knsupc,&INFO);

		  local_dtrtri("U","N",&knsupc,Uinv,&knsupc,&INFO);
