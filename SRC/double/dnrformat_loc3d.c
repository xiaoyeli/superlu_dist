 ...
 )
{
    ...
    if (natural_z_norowperm) {
        ...
        if (Fact == DOFACT) {
            ...
            A2d->nzval = doubleMalloc_dist(nnz_disp[npdep]);
        ...
        if (Fact == DOFACT || Fact == SamePattern || Fact == SamePattern_SameRowPerm) {
            ...
            double *A2d_nzval = (double *) A2d->nzval;
            int_t out_nnz = 0;
        ...
        if (nrhs > 0) {
            if (options && options->GPURES == YES) {
                ...
                checkGPU(gpuMalloc((void**)&Ball,
                                   sizeof(double) * (size_t)all_row_disp[nprocs3d] * (size_t)nrhs));
                checkGPU(gpuMalloc((void**)&A3d->B2d,
                                   sizeof(double) * (size_t)A2d->m_loc * (size_t)nrhs));

                MPI_Allgatherv(Btmp, nrhs * A->m_loc, MPI_DOUBLE,
                ...
                for (int z = 0; z < npdep; ++z) {
                    int src_rank = base_rank + z;
                    ddevice_matcopy_wrap(row_counts_int[z], nrhs,
                                         ((double*)A3d->B2d) + row_disp[z], A2d->m_loc,
                ...
                double *Btmp = SUPERLU_MALLOC(A->m_loc * nrhs * sizeof(double));
                matCopy(A->m_loc, nrhs, Btmp, A->m_loc, B, ldb);

                double *Ball = doubleMalloc_dist(all_row_disp[nprocs3d] * nrhs);
                A3d->B2d = doubleMalloc_dist(A2d->m_loc * nrhs);
