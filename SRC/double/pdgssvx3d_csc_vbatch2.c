...
void dvbatch_ctx_init(dvbatch_ctx_t *ctx)
{
    memset(ctx, 0, sizeof(dvbatch_ctx_t));
} /* end dvbatch_ctx_init */

/*! \brief Release everything pdgssvx3d_csc_vbatch2() kept alive in the context.
 *
 * Safe to call on a context that was only initialized, or twice in a row.
 */
void dvbatch_ctx_free(dvbatch_ctx_t *ctx)
{
    if ( !ctx->initialized ) {
 ...
 * @param[in,out]  ctx persistent context, zeroed by dvbatch_ctx_init() before
 *     the DOFACT call and released by dvbatch_ctx_free() after the last call
 ...
 */
int
pdgssvx3d_csc_vbatch2(
		superlu_dist_options_t *options, /* options for algorithm choices and algorithm parameters */
		dvbatch_ctx_t *ctx, /* persistent state across the calls sharing a pattern */
		...
		)
{
    ...
    *info = 0;
	...
	fprintf(stderr,
		"pdgssvx3d_csc_vbatch2: only Fact = DOFACT (first call) and "
	...
	*info = -8;
	...
	fprintf(stderr, "pdgssvx3d_csc_vbatch2: SamePattern_SameRowPerm asked "
	...
	*info = -2;
	...
	fprintf(stderr, "pdgssvx3d_csc_vbatch2: batchCount/nrhs (%d/%d) differ "
		"from the ones the context was built with (%d/%d).\n",
		batchCount, nrhs, ctx->batchCount, ctx->nrhs);
	*info = -2;
    }
    else if (!reuse && ctx->initialized) {
	fprintf(stderr, "pdgssvx3d_csc_vbatch2: Fact = DOFACT on a context that "
		"already holds a factorization; call dvbatch_ctx_free() "
		"first.\n");
	*info = -2;
    }
    if (*info) {
	pxerr_dist("pdgssvx3d_csc_vbatch2", &(grid3d->grid2d), -(*info));
	return -1;
    }

#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(grid3d->iam, "Enter pdgssvx3d_csc_vbatch2()");
#endif
	...
	fprintf(stderr, "pdgssvx3d_csc_vbatch2: the stacked system changed "
	...
	*/
    ...
    /* NOTE: unlike pdgssvx3d_csc_vbatch(), the L/U factors, the stacked matrix
       ...
       what the next SamePattern_SameRowPerm call reuses.  dvbatch_ctx_free()
       releases them. */
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(grid3d->iam, "Exit pdgssvx3d_csc_vbatch2()");
#endif

    return 0;
} /* end pdgssvx3d_csc_vbatch2 */
