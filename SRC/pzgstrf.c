...
/*
    ...
    Purpose : For SuperNode Level profiling of various measurements such as
   gigaflop/sec obtained,bandwidth achieved: Overhead : Low
*/
 ...
 * </pre>
 */
int_t pzgstrf(superlu_dist_options_t *options, int m, int n, double anorm,
              zLUstruct_t *LUstruct, gridinfo_t *grid, SuperLUStat_t *stat,
              int *info) {
#ifdef _CRAY
  ...
  int_t fnz, i, ib, ijb, ilst, it, iukp, jb, jj, klst, knsupc, lb, lib, ldv,
      ljb, lptr, lptr0, lptrj, luptr, luptr0, luptrj, nlb, nub, nsupc, rel,
      rukp, il, iu;
  int_t Pc, Pr;
  ...
  int_t *etree_supno_l, *etree_supno, *blocks, *blockr, *Ublock, *Urows,
      *Lblock, *Lrows, *perm_u, *sf_block, *sf_block_l, *nnodes_l, *nnodes_u,
      *edag_supno_l, *recvbuf, **edag_supno;
  ...
  int_t *factored;  /* factored[j] == 0 : L col panel j is factorized. */
  int_t *factoredU; /* factoredU[i] == 1 : U row panel i is factorized. */
  ...
  *info = 0;
  ...
  if (Pr * Pc > 1) {
    ...
    if (i != 0) {
      if (!(Llu->Lval_buf_2[0] =
                doublecomplexMalloc_dist((num_look_aheads + 1) * ((size_t)i))))
    ...
    if (i != 0) {
      if (!(Llu->Uval_buf_2[0] =
                doublecomplexMalloc_dist((num_look_aheads + 1) * i)))
  ...
  if (!(Lsub_buf_2 =
            (int_t **)SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(int_t *))))
    ABORT("Malloc fails for Lsub_buf_2[].");
  if (!(Lval_buf_2 = (doublecomplex **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                      sizeof(doublecomplex *))))
    ABORT("Malloc fails for Lval_buf_2[].");
  if (!(Usub_buf_2 =
            (int_t **)SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(int_t *))))
    ABORT("Malloc fails for Uval_buf_2[].");
  if (!(Uval_buf_2 = (doublecomplex **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                      sizeof(doublecomplex *))))
  ...
  if (!(msgcnts =
            (int **)SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(int *))))
  ...
  if (!(recv_reqs_u = (MPI_Request **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                     sizeof(MPI_Request *))))
    ABORT("Malloc fails for recv_reqs_u[].");
  if (!(send_reqs_u = (MPI_Request **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                     sizeof(MPI_Request *))))
    ABORT("Malloc fails for send_reqs_u[].");
  if (!(send_reqs = (MPI_Request **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                   sizeof(MPI_Request *))))
    ABORT("Malloc fails for send_reqs_u[].");
  if (!(recv_reqs = (MPI_Request **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                                   sizeof(MPI_Request *))))
  ...
  if (!(factored = (int_t *)SUPERLU_MALLOC(nsupers * sizeof(int_t))))
    ABORT("Malloc fails for factored[].");
  if (!(factoredU = (int_t *)SUPERLU_MALLOC(nsupers * sizeof(int_t))))
...
#pragma omp master
  ...
  blk_ldu = (int *)SUPERLU_MALLOC(
      (ncb + 1) * sizeof(int)); // +1 to accommodate un-even division
#endif
  ...
  zstatic_schedule(options, m, n, LUstruct, grid, stat, perm_c_supno,
                   iperm_c_supno, info);
  ...
  for (i = 0; i < nsupers; ++i)
    perm_c_supno[i] = iperm_c_supno[i] = i;
#endif
  /* ################################################################## */
  ...
  for (lb = 0; lb < nrb; ++lb) { /* go through U-factor */
    ...
    if (index) { /* Not an empty row */
      ...
      for (j = 0; j < index[0]; ++j) {
        ...
        if (jb != ib)
          look_ahead_l[jb] = SUPERLU_MAX(iperm_c_supno[ib], look_ahead_l[jb]);
...
#pragma omp parallel for reduction(max : local_max_row_size) private(lk, lsub)
  ...
  if (!iam) {
    ...
    printf("\t.. SUPERLU_N_GEMM: %d flops of GEMM done on CPU (1st block "
           "always on CPU)\n",
  ...
  int_t buffer_size = SUPERLU_MAX(max_row_size * nstreams * gpublas_nb,
                                  sp_ienv_dist(8, options));
  //   get_max_buffer_size());
  ...
  if (superlu_acc_offload) {
...
#if (PRNTlevel >= 1)
    if (iam == 0) {
      printf("[%d].. BIG V size " IFMT " (on CPU), dC buffer_size " IFMT
             " (on GPU)\n",
    ...
    // if (checkGPU(
    //         gpuMallocHost((void **)&bigV, bigv_size * sizeof(doublecomplex))))
    //   ABORT("Malloc fails for zgemm buffer V");
    bigV = new doublecomplex[bigv_size];
...
#if defined(HAVE_CUDA) || defined(HAVE_HIP)
    handle =
        (gpublasHandle_t *)SUPERLU_MALLOC(sizeof(gpublasHandle_t) * nstreams);
    ...
    gpuStat = gpuMalloc((void **)&dA, max_row_size * sp_ienv_dist(3, options) *
                                          sizeof(doublecomplex));
    ...
    // Note: handles are not needed for SYCL oneMKL

    // retrieve stream, and several events
    streams = sycl_get_queue();
    gpuEvent_t *events = new gpuEvent_t[nstreams];

    // allocate device memory
    dA = sycl::malloc_device<doublecomplex>(max_row_size * sp_ienv_dist(3, options),
                                     *(sycl_get_queue()));
    if (dA != nullptr) {
      fprintf(stderr, "!!!! Error in allocating A in the device %ld \n",
              m * k * sizeof(doublecomplex));
    ...
    // size of B should be bigu_size
    dB = sycl::malloc_device<doublecomplex>(bigu_size, *(sycl_get_queue()));
    if (dB != nullptr) {
      fprintf(stderr, "!!!! Error in allocating B in the device %ld \n",
              n * k * sizeof(doublecomplex));
      return 1;
    ...
    dC = sycl::malloc_device<doublecomplex>(buffer_size, *(sycl_get_queue()));
    if (dC != nullptr) {
      fprintf(stderr, "!!!! Error in allocating C in the device \n");
      return 1;
    }
  ...
  //     bigU = _mm_malloc(bigu_size * sizeof(doublecomplex), 1<<12); // align
  //     at 4K page bigV = _mm_malloc(bigv_size * sizeof(doublecomplex), 1<<12);
  // #else
  if (!(bigU = doublecomplexMalloc_dist(bigu_size)))
  ...
  lookAhead_L_buff =
      doublecomplexMalloc_dist(ldt * ldt * (num_look_aheads + 1));
  ...
  Remain_L_buff =
      doublecomplexMalloc_dist(Llu->bufmax[1] + j); /* This is loose */
  ...
  if (mycol == kcol) {
    ...
    for (pj = 0; pj < Pc; ++pj) {
      if (ToSendR[lk][pj] != EMPTY) {
        ...
        MPI_Isend(lusup, msgcnt[1], SuperLU_MPI_DOUBLE_COMPLEX, pj,
                  SLU_MPI_TAG(1, 0) /* 1 */, scp->comm, &send_req[pj + Pc]);
#if (DEBUGlevel >= 2)
        printf("[%d] first block cloumn Send L(:,%4d): lsub %4d, lusup %4d to "
               "Pc %2d\n",
    ...
    if (ToRecv[k] >= 1) { /* Recv block column L(:,0). */
      ...
      MPI_Irecv(Lsub_buf_2[0], Llu->bufmax[0], mpi_int_t, kcol,
                SLU_MPI_TAG(0, 0) /* 0 */, scp->comm, &recv_req[0]);
      MPI_Irecv(Lval_buf_2[0], Llu->bufmax[1], SuperLU_MPI_DOUBLE_COMPLEX, kcol,
                SLU_MPI_TAG(1, 0) /* 1 */, scp->comm, &recv_req[1]);
#if (PROFlevel >= 1)
      TOC(t2, t1);
      stat->utime[COMM] += t2;
      stat->utime[COMM_RIGHT] += t2;
  ...
  if (myrow != krow) {
    if (ToRecv[k] == 2) { /* Recv block row U(k,:). */
      ...
      MPI_Irecv(Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
                SLU_MPI_TAG(2, 0) /* 2%tag_ub */, scp->comm,
                &recv_reqs_u[0][0]);
      MPI_Irecv(Uval_buf, Llu->bufmax[3], SuperLU_MPI_DOUBLE_COMPLEX, krow,
                SLU_MPI_TAG(3, 0) /* 3%tag_ub */, scp->comm,
                &recv_reqs_u[0][1]);
#if (PROFlevel >= 1)
      TOC(t2, t1);
      stat->utime[COMM] += t2;
  ...
  for (k0 = 0; k0 < nsupers; ++k0) {
    ...
    for (kk0 = kk1; kk0 <= kk2; kk0++) {
      ...
      if (look_ahead[kk] < k0) { /* does not depend on current column k */
        ...
        if (mycol == kcol) { /* I own this panel */
          ...
          if (lsub1) {
            msgcnt[0] = lsub1[1] + BC_HEADER +
                        lsub1[0] * LB_DESCRIPTOR; /* size of metadata */
...
#endif
#if (DEBUGlevel >= 2)
              printf("[%d] -1- Send L(:,%4d): #lsub1 %4d, #lusup1 %4d right to "
                     "Pj %2d\n",
...
#endif
          MPI_Irecv(Usub_buf, Llu->bufmax[2], mpi_int_t, krow,
                    SLU_MPI_TAG(2, kk0) /* (4*kk0+2)%tag_ub */, scp->comm,
                    &recv_reqs_u[look_id][0]);
          MPI_Irecv(Uval_buf, Llu->bufmax[3], SuperLU_MPI_DOUBLE_COMPLEX, krow,
                    SLU_MPI_TAG(3, kk0) /* (4*kk0+3)%tag_ub */, scp->comm,
                    &recv_reqs_u[look_id][1]);
#if (PROFlevel >= 1)
          TOC(t2, t1);
          stat->utime[COMM] += t2;
          stat->utime[COMM_DOWN] += t2;
    ...
    for (kk0 = kk1; kk0 < kk2; kk0++) {
      ...
      if (factoredU[kk0] != 1 && look_ahead[kk] < k0) {
        ...
        krow = PROW(kk, grid);
        lk = LBj(
            kk, grid); /* Local block number across row. NOT USED?? -- Sherry */
...
#endif
            {
              pzgstrs2_omp(kk0, kk, Glu_persist, grid, Llu, Ublock_info, stat);
            }
...
#if (DEBUGlevel >= 2)
          printf("\t[%d] k=%d, look_id=%d, recv_req[0] == MPI_REQUEST_NULL, "
                 "msgcnt[0] = %d\n",
...
#if (DEBUGlevel >= 2)
          printf("\t[%d] k=%d, look_id=%d, recv_req[1] == MPI_REQUEST_NULL, "
                 "msgcnt[1] = %d\n",
...
/* #pragma omp parallel */ /* Sherry -- parallel done inside pzgstrs2 */
#endif
        { pzgstrs2_omp(k0, k, Glu_persist, grid, Llu, Ublock_info, stat); }
        pdgstrs2_timer += SuperLU_timer_() - ttt2;
    ...
    if (msg0 && msg2) {    /* L(:,k) and U(k,:) are not empty. */
      ...
      for (jj = 0; jj < nub; jj++) {
#ifdef ISORT
        iperm_u[jj] = iperm_c_supno[usub[iukp]]; /* Global block number of block
                                                    U(k,j). */
        perm_u[jj] = jj;
#else
        perm_u[2 * jj] = iperm_c_supno[usub[iukp]]; /* Global block number of
                                                       block U(k,j). */
        perm_u[2 * jj + 1] = jj;
#endif
...
#include "zSchCompUdt-2Ddynamic.c"
  ...
  log_memory(
      -((Llu->bufmax[0] + Llu->bufmax[2]) * (num_look_aheads + 1) * iword +
  ...
  stat->utime[COMM] += t2;
  {
    ...
    MPI_Reduce(&msg_cnt, &msg_cnt_sum, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
    MPI_Reduce(&msg_cnt, &msg_cnt_max, 1, MPI_FLOAT, MPI_MAX, 0, grid->comm);
    MPI_Reduce(&msg_vol, &msg_vol_sum, 1, MPI_FLOAT, MPI_SUM, 0, grid->comm);
    MPI_Reduce(&msg_vol, &msg_vol_max, 1, MPI_FLOAT, MPI_MAX, 0, grid->comm);
    if (iam == 0) {
      printf("\tPZGSTRF comm stat:"
             "\tAvg\tMax\t\tAvg\tMax\n"
             "\t\t\tCount:\t%.0f\t%.0f\tVol(MB)\t%.2f\t%.2f\n",
             msg_cnt_sum / Pr / Pc, msg_cnt_max, msg_vol_sum / Pr / Pc * 1e-6,
             msg_vol_max * 1e-6);
      printf("\t\tcomm time on task 0: %8.2lf\n"
             "\t\t\tcomm down DIAG block %8.2lf\n"
             "\t\t\tcomm right L panel %8.2lf\n"
             "\t\t\tcomm down U panel %8.2lf\n",
             stat->utime[COMM], stat->utime[COMM_DIAG], stat->utime[COMM_RIGHT],
             stat->utime[COMM_DOWN]);
      // #include <float.h>
      // int Digs = DECIMAL_DIG;
      printf("gemm_count %d\n", gemm_count);
      for (i = 0; i < gemm_count; ++i)
        fprintf(fgemm, "%8d%8d%8d\t %20.16e\t%8d\n", gemm_stats[i].m,
                gemm_stats[i].n, gemm_stats[i].k, gemm_stats[i].microseconds,
                prof_sendR[i]);
