...
// #define THREAD_BLOCK_SIZE  256  /* Sherry: was 192. should be <=
// MAX_SUPER_SIZE */

__device__ inline void
ddevice_scatter_l_2D(int thread_id, int nsupc, int temp_nbrow, int_t *usub,
                     int iukp, int_t klst, double *nzval, int ldv,
                     const double *tempv, int nbrow, int *indirect2_thread,
                     int nnz_cols, int ColPerBlock, int *IndirectJ3) {
  int i;
...
__device__ inline void
device_scatter_u_2D(int thread_id, int temp_nbrow, int nsupc, double *ucol,
                    int_t *usub, int iukp, int_t ilst, int_t klst, int_t *index,
                    int iuip_lib, double *tempv, int nbrow, int *indirect,
                    int nnz_cols, int ColPerBlock, int *IndirectJ1,
                    int *IndirectJ3) {
  int i;
...
__global__ void Scatter_GPU_kernel(
    int_t streamId, int_t ii_st, int_t ii_end, int_t jj_st,
    int_t jj_end,          /* defines rectangular Schur block to be scatter */
    int_t klst, int_t jj0, /* 0 on entry */
    int_t nrows, int_t ldt, int_t npcol, int_t nprow, dLUstruct_gpu_t *A_gpu
#ifdef HAVE_SYCL
    ,
    sycl::nd_item<3>& item,
    sycl::local_ptr<int> s,
    sycl::local_ptr<int> ljb_ind,
    sycl::local_ptr<int> ljb_ind    
#endif
  ) {
  ...
  int lb = static_cast<int>(item.get_group(2)) + ii_st;
  int j = static_cast<int>(item.get_group(1)) + jj_st;  
  int blockDim_x = static_cast<int>(item.get_local_range(2));
  ...
  if (jj_st == jj0) {
    tempv1 =
      (j == jj_st) ? bigV : bigV + Ublock_info[j - 1].full_u_cols * nrows;
  } else {
    tempv1 = (j == jj_st) ? bigV
      : bigV + (Ublock_info[j - 1].full_u_cols -
		Ublock_info[jj_st - 1].full_u_cols) *
      nrows;
  }

  /* # of nonzero columns in block j  */
  int nnz_cols =
      (j == 0) ? Ublock_info[j].full_u_cols
  ...
  if (ii_st == 0) {
    ...
    cum_nrow = (lb == 0 ? 0
                        : Remain_info[lb - 1].FullRow -
                              Remain_info[ii_st - 1].FullRow);
  }
  ...
  if (ib < jb) /*scatter U code */
  {
    ...
    /* each thread is assigned a chunk of consecutive U blocks to search */
    {
      ...
      if (thread_id * blks_per_threads + i < num_u_blocks &&
          local_u_blk_infoVec[local_u_blk_infoPtr[lib] +
                              thread_id * blks_per_threads + i]
                  .ljb == ljb) {
    ...
    if (thread_id < nnz_cols)
      IndirectJ3[thread_id] =
          A_gpu->scubufs[streamId].usub_IndirectJ3[cum_ncol + thread_id];
    ...
    device_scatter_u_2D(thread_id, temp_nbrow, nsupc, ucol, usub, iukp, ilst,
                        klst, index, iuip_lib, tempv1, nrows, indirect_lptr,
                        nnz_cols, ColPerBlock, IndirectJ1, IndirectJ3);
  ...
  } else /* ib >= jb, scatter L code */
  {
    ...
    for (int i = 0; i < blks_per_threads; ++i) {
      if (thread_id * blks_per_threads + i < num_l_blocks &&
          local_l_blk_infoVec[local_l_blk_infoPtr[ljb] +
                              thread_id * blks_per_threads + i]
                  .lib == lib) {
    ...
    if (thread_id < nnz_cols)
      IndirectJ3[thread_id] =
          (int)A_gpu->scubufs[streamId].usub_IndirectJ3[cum_ncol + thread_id];
    #if defined(HAVE_SYCL)
    item.barrier(sycl::access::fence_space::local_space);
    #elif defined(HAVE_CUDA) || defined(HAVE_HIP)
    __syncthreads();
    ...
    ddevice_scatter_l_2D(thread_id, nsupc, temp_nbrow, usub, iukp, klst, nzval,
                         ldv, tempv1, nrows, indirect2_thread, nnz_cols,
                         ColPerBlock, IndirectJ3);
  } /* end else ib >= jb */
...
int dSchurCompUpdate_GPU(
    ...
    int_t klst, int_t knsupc, int_t Rnbrow, int_t RemainBlk,
    int_t Remain_lbuf_send_size, int_t bigu_send_size, int_t ldu,
    int_t mcb, /* num_u_blks_hi */
    int_t buffer_size, int_t lsub_len, int_t usub_len, int_t ldt, int_t k0,
    dsluGPU_t *sluGPU, gridinfo_t *grid, SuperLUStat_t *stat) {
  ...
  stat->cPCIeH2D +=
      Remain_lbuf_send_size * sizeof(double) + bigu_send_size * sizeof(double) +
      RemainBlk * sizeof(Remain_info_t) + mcb * sizeof(Ublock_info_t) +
      lsub_len * sizeof(int_t) + usub_len * sizeof(int_t);
  ...
  while (ii_end < RemainBlk) {
    ...
    // nrow_max = Rnbrow;
    int_t remaining_rows =
        (ii_st == 0) ? Rnbrow : Rnbrow - Remain_info[ii_st - 1].FullRow;
    nrow_max = (remaining_rows / nrow_max) > 0
                   ? remaining_rows / CEILING(remaining_rows, nrow_max)
                   : nrow_max;
    ...
    while (jj_end < nub && nrows > 0) {
      int_t remaining_cols = (jj_st == jj_cpu)
                                 ? ncols
                                 : ncols - Ublock_info[jj_st - 1].full_u_cols;
      ...
      if (nrows > 0 && ldu > 0 && ncols > 0) {
        ...
        gpublasDgemm(gpublas_handle0, GPUBLAS_OP_N, GPUBLAS_OP_N, nrows, ncols,
                     ldu, &alpha,
                     &A_gpu->scubufs[streamId]
                          .Remain_L_buff[(knsupc - ldu) * Rnbrow + st_row],
                     Rnbrow, &A_gpu->scubufs[streamId].bigU[st_col * ldu], ldu,
...
#warning this function is synchronous
        ...
        FunCallStream->submit([&](sycl::handler &cgh) {
          localAcc localmem(sycl::range<1>{4 * ldt + 2 * SCATTER_THREAD_BLOCK_SIZE}, cgh);
        ...
        Scatter_GPU_kernel<<<dimGrid, dimBlock,
                             (4 * ldt + 2 * SCATTER_THREAD_BLOCK_SIZE) *
                                 sizeof(int),
                             FunCallStream>>>(streamId, ii_st, ii_end, jj_st,
                                              jj_end, klst, 0, nrows, ldt,
                                              npcol, nprow, dA_gpu);
...
int dfree_LUstruct_gpu(dsluGPU_t *sluGPU, SuperLUStat_t *stat) {
  dLUstruct_gpu_t *A_gpu = sluGPU->A_gpu;
  int streamId = 0;
...
void dPrint_matrix(char *desc, int_t m, int_t n, double *dA, int_t lda) {
  ...
  checkGPU(
      gpuMemcpy(cPtr, dA, lda * n * sizeof(double), gpuMemcpyDeviceToHost));

  int_t i, j;
  printf("\n %s\n", desc);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
      printf(" %.3e", cPtr[i + j * lda]);
    printf("\n");
  }
  free(cPtr);
}
...
int dinitSluGPU3D_t(
    ...
    dLUstruct_t *LUstruct, gridinfo3d_t *grid3d, int_t *perm_c_supno, int_t n,
    int_t buffer_size, /* read from env variable SUPERLU_MAX_BUFFER_SIZE */
    int_t bigu_size, int_t ldt, /* SUPERLU_MAXSUP read from sp_ienv(3) */
    SuperLUStat_t *stat) {
  ...
  if (grid3d->iam == 0) {
    printf(
        "dinitSluGPU3D_t: Using hardware acceleration, with %d gpu streams \n",
        sluGPU->nGPUStreams);
    fflush(stdout);
  ...
  dCopyLUToGPU3D(isNodeInMyGrid, Llu, /* referred to as A_host */
...
int dinitD2Hreduce(int next_k, d2Hreduce_t *d2Hred, int last_flag, HyP_t *HyP,
                   dsluGPU_t *sluGPU, gridinfo_t *grid, dLUstruct_t *LUstruct,
                   SCT_t *SCT) {
  Glu_persist_t *Glu_persist = LUstruct->Glu_persist;
  dLocalLU_t *Llu = LUstruct->Llu;
  ...
  // int_t next_k = perm_c_supno[next_col];  /* global block number for next
  // colum*/
  int_t mkcol, mkrow;
  ...
  if (mkcol == mycol && Lrowind_bc_ptr[kljb] != NULL && last_flag) {
    if (HyP->Lblock_dirty_bit[kljb] > -1) {
      ...
      if (sluGPU->lastOffloadStream[streamIdk0Offload] == lastk0 &&
          lastk0 != -1) {
...
int dreduceGPUlu(int last_flag, d2Hreduce_t *d2Hred, dsluGPU_t *sluGPU,
                 SCT_t *SCT, gridinfo_t *grid, dLUstruct_t *LUstruct) {
  dLocalLU_t *Llu = LUstruct->Llu;
  int iam = grid->iam;
  int_t myrow = MYROW(iam, grid);
  int_t mycol = MYCOL(iam, grid);
...
int dsendLUpanelGPU2HOST(int_t k0, d2Hreduce_t *d2Hred, dsluGPU_t *sluGPU,
                         SuperLUStat_t *stat) {
  int_t kljb = d2Hred->kljb;
  int_t kijb = d2Hred->kijb;
  int_t copyL_kljb = d2Hred->copyL_kljb;
  ...
  if (copyL_kljb)
    checkGPU(gpuMemcpyAsync(
        A_gpu->acc_L_buff, &A_gpu->LnzvalVec[A_gpu->LnzvalPtr_host[kljb]],
        l_copy_len * sizeof(double), gpuMemcpyDeviceToHost, CopyStream));

  if (copyU_kljb)
    checkGPU(gpuMemcpyAsync(
        A_gpu->acc_U_buff, &A_gpu->UnzvalVec[A_gpu->UnzvalPtr_host[kijb]],
...
int dsendSCUdataHost2GPU(int_t streamId, int_t *lsub, int_t *usub, double *bigU,
                         int_t bigu_send_size, int_t Remain_lbuf_send_size,
                         dsluGPU_t *sluGPU, HyP_t *HyP) {
...
void dCopyLUToGPU3D(
    ...
    Glu_persist_t *Glu_persist, int_t n, gridinfo3d_t *grid3d,
    int_t buffer_size, /* bigV size on GPU for Schur complement update */
    int_t bigu_size, int_t ldt, SuperLUStat_t *stat) {
  ...
  stat->ePCIeD2H_Start =
      (gpuEvent_t *)SUPERLU_MALLOC(sizeof(gpuEvent_t) * nsupers);
  stat->ePCIeD2H_End =
      (gpuEvent_t *)SUPERLU_MALLOC(sizeof(gpuEvent_t) * nsupers);
  ...
  for (int_t i = 0; i < CEILING(nsupers, Pc); ++i) {
    ...
    if (A_host->Lrowind_bc_ptr[i] != NULL &&
        isNodeInMyGrid[i * Pc + mycol] == 1) {
  ...
  /*allocating memory*/
  local_l_blk_infoVec = (local_l_blk_info_t *)malloc(
      cum_num_l_blocks * sizeof(local_l_blk_info_t));
  ...
  for (int_t i = 0; i < CEILING(nsupers, Pc); ++i) {
    if (A_host->Lrowind_bc_ptr[i] != NULL &&
      ...
      if (num_l_blocks > 0) {
        ...
        local_l_blk_info_t *local_l_blk_info_i =
            local_l_blk_infoVec + cum_num_l_blocks;
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->local_l_blk_infoVec), local_l_blk_infoVec,
                           cum_num_l_blocks * sizeof(local_l_blk_info_t),
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->local_l_blk_infoPtr), local_l_blk_infoPtr,
                           CEILING(nsupers, Pc) * sizeof(int_t),
                           gpuMemcpyHostToDevice));
  ...
  for (int_t i = 0; i < CEILING(nsupers, Pr); ++i) {
    ...
    if (A_host->Ufstnz_br_ptr[i] != NULL &&
        isNodeInMyGrid[i * Pr + myrow] == 1) {
  ...
  local_u_blk_infoVec = (local_u_blk_info_t *)malloc(
      cum_num_u_blocks * sizeof(local_u_blk_info_t));
  ...
  for (int_t i = 0; i < CEILING(nsupers, Pr); ++i) {
    if (A_host->Ufstnz_br_ptr[i] != NULL &&
      ...
      if (num_u_blocks > 0) {
        local_u_blk_info_t *local_u_blk_info_i =
            local_u_blk_infoVec + cum_num_u_blocks;
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->local_u_blk_infoVec), local_u_blk_infoVec,
                           cum_num_u_blocks * sizeof(local_u_blk_info_t),
                           gpuMemcpyHostToDevice));
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->local_u_blk_infoPtr), local_u_blk_infoPtr,
                           CEILING(nsupers, Pr) * sizeof(int_t),
                           gpuMemcpyHostToDevice));
  ...
  printf("(%d) Number of L blocks in GPU %d, U blocks %d\n", grid3d->iam,
         num_gpu_l_blocks, num_gpu_u_blocks);
  printf(
      "(%d) elimination order of first block in GPU: L block %d, U block %d\n",
      grid3d->iam, A_gpu->first_l_block_gpu, A_gpu->first_u_block_gpu);
  printf("(%d) Memory of L %.1f GB, memory for U %.1f GB, Total device memory "
         "used %.1f GB, Memory allowed %.1f GB \n",
         grid3d->iam, l_val_len * sizeof(double) * 1e-9,
         u_val_len * sizeof(double) * 1e-9, gpu_mem_used * 1e-9,
         max_gpu_memory * 1e-9);
  fflush(stdout);
#endif
  ...
  for (int_t jb = 0; jb < nsupers; ++jb) /* for each block column ... */
  {
    ...
    if (mycol == pc && isNodeInMyGrid[jb] == 1) {
      ...
      if (index_host != NULL) {
        ...
        memcpy(&indtemp[temp_LrowindPtr[ljb]], index_host,
               len1 * sizeof(int_t));
      }
    }
  }
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->LrowindVec), indtemp,
                           l_ind_len * sizeof(int_t), gpuMemcpyHostToDevice));
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->LrowindPtr), temp_LrowindPtr,
                           l_k * sizeof(int_t), gpuMemcpyHostToDevice));

  checkGPUErrors(gpuMalloc(&tmp_ptr, l_k * sizeof(int_t)));
  A_gpu->LnzvalPtr = (int_t *)tmp_ptr;
  checkGPUErrors(gpuMemcpy((A_gpu->LnzvalPtr), temp_LnzvalPtr,
                           l_k * sizeof(int_t), gpuMemcpyHostToDevice));
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->UrowindVec), indtemp1,
                           u_ind_len * sizeof(int_t), gpuMemcpyHostToDevice));
  ...
  checkGPUErrors(gpuMemcpy((A_gpu->UrowindPtr), temp_UrowindPtr,
                           u_k * sizeof(int_t), gpuMemcpyHostToDevice));

  A_gpu->UnzvalPtr_host = temp_UnzvalPtr;

  checkGPUErrors(gpuMalloc(&tmp_ptr, u_k * sizeof(int_t)));
  A_gpu->UnzvalPtr = (int_t *)tmp_ptr;
  checkGPUErrors(gpuMemcpy((A_gpu->UnzvalPtr), temp_UnzvalPtr,
                           u_k * sizeof(int_t), gpuMemcpyHostToDevice));

  checkGPUErrors(gpuMalloc(&tmp_ptr, (nsupers + 1) * sizeof(int_t)));
  A_gpu->xsup = (int_t *)tmp_ptr;
  checkGPUErrors(gpuMemcpy((A_gpu->xsup), xsup, (nsupers + 1) * sizeof(int_t),
                           gpuMemcpyHostToDevice));

  checkGPUErrors(gpuMalloc(&tmp_ptr, sizeof(dLUstruct_gpu_t)));
  *dA_gpu = (dLUstruct_gpu_t *)tmp_ptr;
  checkGPUErrors(gpuMemcpy(*dA_gpu, A_gpu, sizeof(dLUstruct_gpu_t),
                           gpuMemcpyHostToDevice));
...
int dreduceAllAncestors3d_GPU(int_t ilvl, int_t *myNodeCount, int_t **treePerm,
                              ...
                              gridinfo3d_t *grid3d, dsluGPU_t *sluGPU,
                              d2Hreduce_t *d2Hred, factStat_t *factStat,
  ...
  if (myGrid == sender && superlu_acc_offload) {
    ...
    for (int_t alvl = ilvl + 1; alvl < maxLvl; ++alvl) {
      ...
      for (int_t node = 0; node < nsAncestor; node++) {
        ...
        if (!gpuLUreduced[k]) {
          dinitD2Hreduce(k, d2Hred, 1, HyP, sluGPU, grid, LUstruct, SCT);
          int_t copyL_kljb = d2Hred->copyL_kljb;
          int_t copyU_kljb = d2Hred->copyU_kljb;
  ...
  dreduceAllAncestors3d(ilvl, myNodeCount, treePerm, LUvsb, LUstruct, grid3d,
                        SCT);
  return 0;
