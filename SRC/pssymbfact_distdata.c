 ...
 * </pre>
 */
float sdist_psymbtonum(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
                       ...
                       sLUstruct_t *LUstruct, gridinfo_t *grid)
{
  ...
  if (!(dense = floatCalloc_dist(SUPERLU_MAX(ldaspa, ldaspa_j) * sp_ienv_dist(3, options))))
  {
  ...
  memTRS += nsupers_j * sizeof(int_t *) + 2.0 * nsupers_j * sizeof(float) + nsupers_j * iword + 5 * nsupers_j * sizeof(long int);
