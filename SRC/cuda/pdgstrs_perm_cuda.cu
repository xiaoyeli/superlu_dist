                                       ...
                                       int_t row_per_th)
{
    ...
    for (int_t rr = 0; rr < row_per_th; ++rr) {
        ...
        if (local_i < m_loc) {
            ...
            send_idbuf[off] = (double) dest_row;
                                       ...
                                       int_t row_per_th)
{
    ...
    for (int_t rr = 0; rr < row_per_th; ++rr) {
        ...
        if (rec_i < recvl) {
            int_t dest_row = (int_t) recv_idbuf[rec_i * ((int_t)nrhs + 1)];
    ...
    gridinfo_t *grid, dSOLVEstruct_t *SOLVEstruct)
{
    ...
    for (int pp = 0; pp < procs; ++pp) {
        ...
        if (sendcnts_nrhs[dst] > 0) {
            ...
            nvshmem_double_put_nbi(d_recv_idbuf_PermuteC + remote_offset,
    ...
    int colequ)
{
    ...
    if (notran) {
        if (rowequ) val *= R[grow];
    } else {
        if (colequ) val *= C[grow];
    }
