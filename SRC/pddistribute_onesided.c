	...
	for (ljb = 0; ljb < k; ++ljb) { /* for each local block column ... */
		...
		if(Iactive==1){
			...
			if(rank_cnt>1){
				...
				//C_BcTree_Create_onesided(&LBtree_ptr[ljb], grid->comm, ranks, rank_cnt, msgsize, 'd');
				C_BcTree_Create_onesided(&LBtree_ptr[ljb], grid->comm, ranks, rank_cnt, msgsize, 'd', BufSize, Pc);
                /* record #msg I need to recv from each process.*/
		...
		if(ib<nsupers){
			...
			if(Iactive==1){
				...
				if(rank_cnt>1){
					...
					C_RdTree_Create_onesided(&LRtree_ptr[lib], grid->comm, ranks, rank_cnt, msgsize, 'd',BufSize_rd,Pc);
...
if ( !iam) printf(".. Construct Reduce tree for L: %.2f\t\n", t);
	...
	// fflush(stdout);
			if(rank_cnt>1){
				...
				C_BcTree_Create_onesided(&UBtree_ptr[ljb], grid->comm, ranks, rank_cnt, msgsize, 'd', BufSize_u, Pc);
		...
		if(ib<nsupers){
			...
			if(Iactive==1){
				...
				if(rank_cnt>1){
					...
					C_RdTree_Create_onesided(&URtree_ptr[lib], grid->comm, ranks, rank_cnt, msgsize, 'd',BufSize_urd, Pc);
