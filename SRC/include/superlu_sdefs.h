 ...
 *  one sparsity pattern; see psgssvx3d_csc_vbatch2().
 *
 * The caller owns the struct; zero it with svbatch_ctx_init() before the first
 * (Fact = DOFACT) call and release it with svbatch_ctx_free() after the last.
 ...
 */
typedef struct {
    int       initialized;  /* 0 until the first psgssvx3d_csc_vbatch2() call */
...
extern int psgssvx3d_csc_vbatch2(
        superlu_dist_options_t *, svbatch_ctx_t *ctx,
...
extern void svbatch_ctx_init(svbatch_ctx_t *ctx);
extern void svbatch_ctx_free(svbatch_ctx_t *ctx);
