...
__device__ inline void ddevice_scatter_l_2D(int thread_id,
...
__device__ inline void device_scatter_u_2D(int thread_id,
	...
	dsluGPU_t *sluGPU, gridinfo_t *grid)
{
	...
	A_gpu->cPCIeH2D += Remain_lbuf_send_size * sizeof(double) + bigu_send_size * sizeof(double) + RemainBlk * sizeof(Remain_info_t) + mcb * sizeof(Ublock_info_t) + lsub_len * sizeof(int_t) + usub_len * sizeof(int_t);
	...
	while (ii_end < RemainBlk)
	{
		...
		while (jj_end < nub && nrows > 0)
		{
			...
			else /* C matrix cannot fit in buffer, need to break into pieces */
			{
				...
				ncol_max = (remaining_cols / ncol_max) > 0 ? remaining_cols / CEILING(remaining_cols, ncol_max)
														   : ncol_max;
...
#warning this function is synchronous
		     ...
		     */
				...
				Scatter_GPU_kernel<<<dimGrid, dimBlock, (4 * ldt + 2 * SCATTER_THREAD_BLOCK_SIZE) * sizeof(int), FunCallStream>>>(streamId, ii_st, ii_end, jj_st, jj_end, klst,
