			...
			if (parSymbFact == NO || Fact == SamePattern_SameRowPerm)
			{
				...
				if(Fact == SamePattern_SameRowPerm){
					...
					for (int jb = 0; jb < CEILING( nsupers, grid->npcol ); ++jb) { /* for each block column ... */
						if ( Llu->Urbs[jb] ) {
							SUPERLU_FREE(Llu->Ucb_indptr[jb]);
							SUPERLU_FREE(Llu->Ucb_valptr[jb]);
						}						
						if(Llu->Linv_bc_ptr[jb]!=NULL)
							SUPERLU_FREE(Llu->Linv_bc_ptr[jb]);
						if(Llu->Uinv_bc_ptr[jb]!=NULL)
							SUPERLU_FREE(Llu->Uinv_bc_ptr[jb]);
						if(Llu->Lindval_loc_bc_ptr[jb]!=NULL)
							SUPERLU_FREE(Llu->Lindval_loc_bc_ptr[jb]);	
						if ( Llu->Lrowind_bc_ptr[jb] ) {
							SUPERLU_FREE (Llu->Lrowind_bc_ptr[jb]);
							SUPERLU_FREE (Llu->Lnzval_bc_ptr[jb]);
						}													
					}
					...
					for (int lb = 0; lb < CEILING( nsupers, grid->nprow ); ++lb) { /* for each block row ... */
						...
						if(Llu->Unzval_br_ptr[lb]!=NULL)
							SUPERLU_FREE(Llu->Unzval_br_ptr[lb]);															
					}
