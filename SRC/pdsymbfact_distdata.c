            ...
            gridinfo_t *grid)
{
  ...
  xlsub_s = Pslu_freeable->xlsub;
  lsub_s = Pslu_freeable->lsub;
  xusub_s = Pslu_freeable->xusub;
  usub_s = Pslu_freeable->usub;
  ...
  for (p = 0; p < nprocs; p++)
  {
    if (p != iam)
    {
      SendCnt_l += nnzToSend[2 * p];
      nnzToSend_l[p] = nnzToSend[2 * p];
      SendCnt_u += nnzToSend[2 * p + 1];
      nnzToSend_u[p] = nnzToSend[2 * p + 1];
      RecvCnt_l += nnzToRecv[2 * p];
      nnzToRecv_l[p] = nnzToRecv[2 * p];
      RecvCnt_u += nnzToRecv[2 * p + 1];
  ...
  while (sendL || sendU)
  {
    if (sendL)
    {
      ...
      sub_s = lsub_s;
      xsub_n = xlsub_n;
      nnzToSend = nnzToSend_l;
      nnzToRecv = nnzToRecv_l;
    }
    if (sendU)
    {
      xsub_s = xusub_s;
      sub_s = usub_s;
      xsub_n = xusub_n;
      nnzToSend = nnzToSend_u;
      nnzToRecv = nnzToRecv_u;
    }
  ...
  if (xlsub_s != NULL)
  {
    SUPERLU_FREE(xlsub_s);
    SUPERLU_FREE(lsub_s);
  }
  if (xusub_s != NULL)
  {
    SUPERLU_FREE(xusub_s);
    SUPERLU_FREE(usub_s);
  }
  SUPERLU_FREE(globToLoc);
  if (supno_s != NULL)
  {
    SUPERLU_FREE(xsup_beg_s);
    SUPERLU_FREE(xsup_end_s);
    SUPERLU_FREE(supno_s);
  }

  Glu_persist->supno = supno_n;
  Glu_persist->xsup = xsup_n;
  *p_xlsub = xlsub_n;
  *p_lsub = lsub_n;
  *p_xusub = xusub_n;
  *p_usub = usub_n;
 ...
 * </pre>
 */
float ddist_psymbtonum(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
                       ...
                       dLUstruct_t *LUstruct, gridinfo_t *grid)
{
  ...
  if (!(dense = doubleCalloc_dist(SUPERLU_MAX(ldaspa, ldaspa_j) * sp_ienv_dist(3, options))))
  {
  ...
  memTRS += nsupers_j * sizeof(int_t *) + 2.0 * nsupers_j * sizeof(double) + nsupers_j * iword + 5 * nsupers_j * sizeof(long int);
  ...
  for (jb = 0; jb < nsupers; jb++)
  {
     ...
     *------------------------------------------------*/
    if (myrow == jbrow)
    { /* Block row jb in my process row */
      ...
      for (i = xusub[ljb_i]; i < xusub[ljb_i + 1]; i++)
      {
        ...
        { /* I own this block */
          ...
          { /* First see this block */
            if (Urb_marker[lb] == FALSE && gb != jb && myrow != pr)
              nbrecvx++;
            Urb_marker[lb] = jb + 1;
            LUb_number[nrbu] = gb;
  ...
  for (ljb = 0; ljb < CEILING(nsupers, grid->npcol); ++ljb)
  {                                 /* for each local block column ... */
    jb = mycol + ljb * grid->npcol; /* not sure */
    if (jb < nsupers)
    {
  ...
  for (lib = 0; lib < CEILING(nsupers, grid->nprow); ++lib)
  {                                 /* for each local block row ... */
    ib = myrow + lib * grid->nprow; /* not sure */
    ...
    if (ib < nsupers)
    {
      for (i = xusub[lib]; i < xusub[lib + 1]; i++)
      {
        ...
        pr = PROW(ib, grid);
        if (mycol == pc)
        { /* Block column ib in my process column */
