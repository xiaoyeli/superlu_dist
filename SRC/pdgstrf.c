...
int_t pdgstrf(superlu_dist_options_t *options, int m, int n, double anorm,
  ...
  *info = 0;
  ...
  if (!(Lval_buf_2 = (double **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                               sizeof(double *))))
    ABORT("Malloc fails for Lval_buf_2[].");
  if (!(Usub_buf_2 =
            (int_t **)SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(int_t *))))
    ABORT("Malloc fails for Uval_buf_2[].");
  if (!(Uval_buf_2 = (double **)SUPERLU_MALLOC((1 + num_look_aheads) *
                                               sizeof(double *))))
...
#pragma omp master
  ...
  dstatic_schedule(options, m, n, LUstruct, grid, stat, perm_c_supno,
                   iperm_c_supno, info);
...
#pragma omp parallel for reduction(max : local_max_row_size) private(lk, lsub)
  ...
  if (superlu_acc_offload) {
    ...
    //checkGPUErrors(gpuMallocHost(&bigU, bigu_size * sizeof(double)));
    ...
    //checkGPUErrors(gpuMallocHost(&bigV, bigv_size * sizeof(double)));
  ...
  //     bigU = _mm_malloc(bigu_size * sizeof(double), 1<<12); // align at 4K
  //     page bigV = _mm_malloc(bigv_size * sizeof(double), 1<<12);
  // #else
  if (!(bigU = doubleMalloc_dist(bigu_size)))
    ABORT("Malloc fails for dgemm U buffer");
  if (!(bigV = doubleMalloc_dist(bigv_size)))
...
#endif
            {
              pdgstrs2_omp(kk0, kk, Glu_persist, grid, Llu, Ublock_info, stat);
            }
