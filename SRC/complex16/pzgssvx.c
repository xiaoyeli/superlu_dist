...
pzgssvx(superlu_dist_options_t *options, SuperMatrix *A,
	...
	zLUstruct_t *LUstruct, zSOLVEstruct_t *SOLVEstruct, double *berr,
...
// if ( 0){
	...
	if ( options->IterRefine || options->UseGMRES ) {
		...
		for (jj_ = 0; jj_ < nrhs; ++jj_) {
		    ...
		    pzgmres(options, n, A, LUstruct, ScalePermstruct, grid,
...
#if ( PRNTlevel>=1 )
		if ( !iam )
		    printf(".. pzgmres direct solve: %d total GMRES iterations\n",
			   gmres_totit);
#endif
...
/////////////////////////  row-wise data structure for GPU U solve (using the highest skyline accross the entire supernode row)
	...
	for (lk = 0; lk < nsupers_i; ++lk) { /* For each block row. */
		...
		if ( usub1 ) { /* Not an empty block row. */
			...
			if (!(Llu->Unzval_br_new_ptr[lk]=doublecomplexCalloc_dist(ncol*LDA)))
	...
	if ( !(Llu->Unzval_br_new_dat =
		(doublecomplex*)SUPERLU_MALLOC(Unzval_br_new_cnt * sizeof(doublecomplex))) ) {
...
if (get_acc_solve()){
	...
	if(Llu->d_Unzval_br_new_dat==NULL)checkGPU(gpuMalloc( (void**)&Llu->d_Unzval_br_new_dat, (Llu->Unzval_br_new_cnt) * sizeof(doublecomplex)));
	checkGPU(gpuMemcpy(Llu->d_Unzval_br_new_dat, Llu->Unzval_br_new_dat, (Llu->Unzval_br_new_cnt) * sizeof(doublecomplex), gpuMemcpyHostToDevice));
	...
	if(Llu->d_Unzval_bc_dat==NULL)checkGPU(gpuMalloc( (void**)&Llu->d_Unzval_bc_dat, (Llu->Unzval_bc_cnt) * sizeof(doublecomplex)));
...
void zpacked2skyline(int_t k, int_t *usubpack, doublecomplex *valpack, int_t *usub, doublecomplex *uval, int_t*xsup)
{
...
void
pzconvertUROWDATA2skyline(superlu_dist_options_t *options, gridinfo_t *grid,
	   zLUstruct_t *LUstruct, SuperLUStat_t *stat, int n)
{
    ...
    for (int_t i = 0; i < CEILING(nsupers, Pr); ++i)
    {
        if (Ufstnz_br_ptr[i] != NULL )
        {
            ...
            zpacked2skyline(globalId, LUstruct->Llu->Ucolind_br_ptr[i], LUstruct->Llu->Unzval_br_new_ptr[i], Ufstnz_br_ptr[i], Unzval_br_ptr[i], xsup);
        }
    }
}
...
void
pzconvert_flatten_skyline2UROWDATA(superlu_dist_options_t *options, gridinfo_t *grid,
	   zLUstruct_t *LUstruct, SuperLUStat_t *stat, int n)
{
int64_t nnz_ind,nnz_offset;
int64_t nnz_val;
...
int pzflatten_LDATA(superlu_dist_options_t *options, int_t n, zLUstruct_t * LUstruct,
                           gridinfo_t *grid)
{
    Glu_persist_t *Glu_persist = LUstruct->Glu_persist;
    int kr,kc,nlb,nub;
