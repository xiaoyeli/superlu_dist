				      ...
				      )
{
    ...
    int iam = grid3d->iam;
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC (iam, "Enter zinitTrf3DpartitionLUstructgrid0()");
#endif
    int_t nsupers;
    int_t *setree;
    if (!grid3d->zscp.Iam){
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC (iam, "Exit zinitTrf3DpartitionLUstructgrid0()");
#endif
...
ztrf3Dpartition_t* zinitTrf3Dpartition_allgrid(int_t n, superlu_dist_options_t *options,
				      zLUstruct_t *LUstruct, gridinfo3d_t * grid3d
				      )
{
    gridinfo_t* grid = &(grid3d->grid2d);
    int iam = grid3d->iam;
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC (iam, "Enter zinitTrf3Dpartition_allgrid()");
#endif
    int_t nsupers;
    int_t *setree;
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC (iam, "Exit zinitTrf3Dpartition_allgrid()");
#endif
    return trf3Dpartition;
