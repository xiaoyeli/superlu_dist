 ...
 *             see supermatrix.h for nrformat_loc3d{} structure.
 */
void zGatherNRformat_loc3d_allgrid
(
 ...
 )
{
	...
	MPI_Allgatherv(A->nzval, A->nnz_loc, SuperLU_MPI_DOUBLE_COMPLEX, A2d->nzval,
		    nnz_counts_int, nnz_disp,
		    SuperLU_MPI_DOUBLE_COMPLEX, grid3d->zscp.comm);
	...
	*A3d_addr = (NRformat_loc3d *) A3d; // return pointer to A3d struct
	...
	if (options && options->GPURES == YES) {
	    ...
	    // B1 <- allgatherv(Btmp)
	    MPI_Allgatherv(Btmp, nrhs * A->m_loc, SuperLU_MPI_DOUBLE_COMPLEX, B1,
			   b_counts_int, b_disp,
			   SuperLU_MPI_DOUBLE_COMPLEX, grid3d->zscp.comm);
	    checkGPU(gpuFree(Btmp));
	    ...
	    for (int i = 0; i < grid3d->npdep; ++i)
	    {
		zdevice_matcopy_wrap(row_counts_int[i], nrhs,
				     ((doublecomplex*)A3d->B2d) + row_disp[i], A2d->m_loc,
 ...
 *   Output: X3d <- A^{-1} B2d
 */
int zScatter_B3d(superlu_dist_options_t *options,
		 ...
		 gridinfo3d_t *grid3d)
{
    ...
    if (gpures && grid3d->npdep == 1) {
#ifdef GPU_ACC
        zdevice_matcopy_wrap(A3d->m_loc, nrhs,
                                  B, ldb,
                                  B2d, A2d->m_loc);
        ...
        return 0;
#else
        ABORT("GPURES requires GPU_ACC in zScatter_B3d().");
#endif
    }

    doublecomplex *B1 = NULL;  // on 2D layer 0
    if (grid3d->zscp.Iam == 0)
    {
	if (gpures) {
#ifdef GPU_ACC
...
#ifdef GPU_ACC
	checkGPU(gpuMalloc((void**)&Btmp,
			   sizeof(doublecomplex) * (size_t)A3d->m_loc * (size_t)nrhs));
#else
	ABORT("GPURES requires GPU_ACC in zScatter_B3d().");
#endif
    } else {
...
(equations     3              1      Proc 0 -> Procs {0, 1, 2, 3}
 ...
 after gather) 9              3
	...
	*/
    ...
    if (gpures) {
#ifdef GPU_ACC
	zdevice_matcopy_wrap(A3d->m_loc, nrhs, B, ldb, Btmp, A3d->m_loc);
#else
	ABORT("GPURES requires GPU_ACC in zScatter_B3d().");
#endif
    } else {
