...
float
pzdistribute_allgrid(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
	     zScalePermstruct_t *ScalePermstruct,
	     Glu_freeable_t *Glu_freeable, zLUstruct_t *LUstruct,
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter pzdistribute_allgrid()");
#endif
#if ( PROFlevel>=1 )
    t = SuperLU_timer_();
#endif
...
/* Count the nnzs per block column */
    ...
    /* Memory allocated but not freed:
       ilsum, fmod, fsendx_plist, bmod, bsendx_plist  */
    CHECK_MALLOC(iam, "Exit pzdistribute_allgrid()");
#endif
...
float
pzdistribute_allgrid_index_only(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
	     zScalePermstruct_t *ScalePermstruct,
	     Glu_freeable_t *Glu_freeable, zLUstruct_t *LUstruct,
	     gridinfo_t *grid, int* supernodeMask)
/*
...
//#endif
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter pzdistribute_allgrid_index_only()");
#endif
#if ( PROFlevel>=1 )
    t = SuperLU_timer_();
#endif
    ...
    /* Memory allocated but not freed:
       ilsum, fmod, fsendx_plist, bmod, bsendx_plist  */
    CHECK_MALLOC(iam, "Exit pzdistribute_allgrid_index_only()");
#endif
