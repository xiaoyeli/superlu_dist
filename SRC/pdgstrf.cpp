        ...
        dLUstruct_t * LUstruct, gridinfo_t * grid, SuperLUStat_t * stat, int *info)
{
    ...
    MPI_Request *recv_req, **recv_reqs, **send_reqs, **send_reqs_u,
        **recv_reqs_u;
    MPI_Request *send_req, *U_diag_blk_send_req = nullptr;
    MPI_Status status;
    void *attr_val;
    int flag;
...
/* #endif */
    ...
    *info = 0;
    ...
    if (!(factored = (int *)SUPERLU_MALLOC (nsupers * sizeof (int_t))))
        ABORT ("Malloc fails for factored[].");
    if (!(factoredU = (int *)SUPERLU_MALLOC (nsupers * sizeof (int_t))))
    ...
    bigU = nullptr; /* allocated only on CPU */
    bigV = nullptr;
    ...
    sycl::context super_ctxt = sycl::context(gpu_devices, asyncHandler);
    ...
    if ( iam==0 ) {
        DisplayHeader();
        printf(" Starting with %d Sycl Streams[0] \n",nstreams[0] );
        fflush(stdout);
    }
#endif
    ...
    dA = sycl::malloc_device<double>(max_row_size*sp_ienv_dist(3), streams[0]);
    if( !dA )
        ABORT("[SYCL] Malloc fails for dA buffer..");
    // size of B should be bigu_size
    dB = sycl::malloc_device<double>(bigu_size, streams[0]);
    if( !dB )
        ABORT("[SYCL] Malloc fails for dB buffer..");
    dC = sycl::malloc_device<double>(buffer_size, streams[0]);
    if( !dC )
...
sycl::free( dA, streams[0] ); /* Sherry added */
