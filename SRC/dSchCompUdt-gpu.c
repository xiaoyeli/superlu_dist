...
#pragma omp for schedule(SCHEDULE_STRATEGY)
      ...
      if (num_streams_used > 0) {
        ...
        gpuMemcpy2DAsync(dA, nbrow * sizeof(double),
                         ...
                         nsupr * sizeof(double), nbrow * sizeof(double), ldu,
      ...
      for (int i = 0; i < num_streams_used; ++i) { // streams on GPU
        ...
        if (num_col_stream > 0) {
          ...
          gpublasCheckErrors(gpublasDgemm(handle[stream_id], GPUBLAS_OP_N,
                                          GPUBLAS_OP_N, nbrow, num_col_stream,
                                          ldu, &alpha, dA, nbrow, &dB[b_offset],
                                          ldu, &beta, &dC[c_offset], nbrow));
          ...
          my_dgemm_("N", "N", &nbrow, &num_col_stream, &ldu, &alpha,
...
#pragma omp for schedule(SCHEDULE_STRATEGY) nowait
            ...
            for (lb = 0; lb < nlb; lb++) {
              ...
              if (ib < jb) { /* A(i,j) is in U. */
                ...
                dscatter_u(ib, jb, nsupc, iukp, xsup, klst, nbrow, lptr,
                ...
                dscatter_l(ib, ljb, nsupc, iukp, xsup, klst, nbrow, lptr,
