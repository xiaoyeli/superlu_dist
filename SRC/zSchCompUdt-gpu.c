...
if (msg0 && msg2) { /* L(:,k) and U(k,:) are not empty. */
  ...
  if (nbrow > 0) {
...
#if 0 // max_ldu can be < ldt, so bigu_size/ldt may be smaller, giving false
      // alarm
    ...
    for (j = jj0; j < nub; ++j) {
      arrive_at_ublock(j, &iukp, &rukp, &jb, &ljb, &nsupc, iukp0, rukp0, usub,
                       perm_u, xsup, grid);
...
#if (PRNTlevel >= 1)
      if (parts > 1) {
        printf("warning: number of partitions %d > 1, try increasing "
               "MAX_BUFFER_SIZE.\n",
               parts);
      }
#endif
...
#pragma omp single
#endif
      {
        ...
        gemm_division_cpu_gpu(
            options, &num_streams_used, /*number of streams that will be used*/
            ...
            nstreams,
            full_u_cols +
                jjj_st,          /*array containing prefix sum of GPU workload*/
...
#pragma omp for schedule(SCHEDULE_STRATEGY)
      ...
      if (num_streams_used > 0) {
        ...
        printf("nbrow %d *ldu %d  =%d < ldt %d * max_row_size %d =%d \n", nbrow,
               ldu, nbrow * ldu, ldt, max_row_size, ldt * max_row_size);
        fflush(stdout);
        ...
        gpuMemcpy2DAsync(dA, nbrow * sizeof(doublecomplex),
                         ...
                         nsupr * sizeof(doublecomplex),
                         nbrow * sizeof(doublecomplex), ldu,
        ...
        gpuMemcpy2DAsync(dA, nbrow * sizeof(doublecomplex),
                         ...
                         nsupr * sizeof(doublecomplex),
                         nbrow * sizeof(doublecomplex), ldu,
                         gpuMemcpyHostToDevice, streams[0]);
#endif
      }
      ...
      for (int i = 0; i < num_streams_used; ++i) { // streams on GPU
        ...
        int num_col_stream =
            full_u_cols[jjj_st + stream_end_col[i] - 1] - full_u_cols[st - 1];
        tempu = bigU;
        ...
        if (num_col_stream > 0) {
          ...
          gpublasCheckErrors(
              gpublasSetStream(handle[stream_id], streams[stream_id]));

          gpublasCheckErrors(
              gpublasZgemm(handle[stream_id], GPUBLAS_OP_N, GPUBLAS_OP_N, nbrow,
                           num_col_stream, ldu, (const cuDoubleComplex *)&alpha,
                           (const cuDoubleComplex *)dA, nbrow,
                           (const cuDoubleComplex *)&dB[b_offset], ldu,
                           (const cuDoubleComplex *)&beta,
                           (cuDoubleComplex *)&dC[c_offset], nbrow));

          checkGPU(gpuMemcpyAsync(tempv1, dC + c_offset, C_stream_size,
                                  gpuMemcpyDeviceToHost, streams[stream_id]));
#elif defined(HAVE_SYCL)
          auto H2D =
              streams->memcpy(dB + b_offset, tempu + b_offset, B_stream_size);

          auto GEMM = oneapi::mkl::blas::gemm(
              *streams, oneapi::mkl::transpose::nontrans,
              oneapi::mkl::transpose::nontrans, nbrow, num_col_stream, ldu,
              alpha, dA, nbrow, &dB[b_offset], ldu, &beta, &dC[c_offset], nbrow,
              std::vector<sycl::event>{H2D});

          auto D2H =
              streams->memcpy(tempv1, dC + c_offset, C_stream_size, GEMM);
          D2H.wait();
          ...
          my_zgemm_("N", "N", &nbrow, &num_col_stream, &ldu, &alpha,
                    &lusup[luptr + (knsupc - ldu) * nsupr], &nsupr,
                    tempu + ldu * st_col, &ldu, &beta, tempv1, &nbrow, 1, 1);
        }
...
#else
    zgemm_("N", "N", &nbrow, &num_col, &ldu, &alpha,
           &lusup[luptr + (knsupc - ldu) * nsupr], &nsupr, tempu + ldu * st_col,
           &ldu, &beta, tempv, &nbrow);
#endif
...
#pragma omp parallel private(                                                  \
    j, iukp, rukp, tempu, tempv, cum_nrow, jb, nsupc, ljb, segsize, lead_zero, \
    ib, temp_nbrow, ilst, lib, index, ijb, fnz, ucol, rel, ldv, lptrj, luptrj, \
    nzval, lb, jj, i) firstprivate(luptr, lptr) default(shared)
#endif
      {
#ifdef _OPENMP
      ...
      int num_threads = 1;
        ...
        if (ncpu_blks < num_threads) {
          ...
          for (j = jjj_st; j < jjj_st + ncpu_blks; ++j) {
            ...
            arrive_at_ublock(j, &iukp, &rukp, &jb, &ljb, &nsupc, iukp0, rukp0,
                             usub, perm_u, xsup, grid);
...
#pragma omp for schedule(SCHEDULE_STRATEGY) nowait
            ...
            for (lb = 0; lb < nlb; lb++) {
              ...
              if (ib < jb) { /* A(i,j) is in U. */
                ...
                zscatter_u(ib, jb, nsupc, iukp, xsup, klst, nbrow, lptr,
                           temp_nbrow, lsub, usub, tempv, Ufstnz_br_ptr,
                           Unzval_br_ptr, grid);
              } else { /* A(i,j) is in L. */
#ifdef PI_DEBUG
                printf("cpu scatter \n");
                ...
                zscatter_l(ib, ljb, nsupc, iukp, xsup, klst, nbrow, lptr,
                           temp_nbrow, usub, lsub, tempv, indirect_thread,
                           indirect2_thread, Lrowind_bc_ptr, Lnzval_bc_ptr,
                           grid);
              } /* if ib < jb ... */
    ...
    nzval, lb, jj, i) firstprivate(luptr, lptr) default(shared)
      ...
      int thread_id = 0;
        ...
        for (i = 0; i < num_streams_used; i++) { /* i is private variable */
          ...
          int jjj_st1 =
              (i == 0) ? jjj_st + ncpu_blks : jjj_st + stream_end_col[i - 1];
