...
// nrow_max = Rnbrow;
		...
		if (nrows > 0 && ldu > 0 && ncols > 0)
		{
		    ...
		    cublasSgemm(cublas_handle0, CUBLAS_OP_N, CUBLAS_OP_N,
...
void sPrint_matrix( char *desc, int_t m, int_t n, float * dA, int_t lda )
{
	...
	checkCuda(cudaMemcpy( cPtr, dA,
	                      lda * n * sizeof(float), cudaMemcpyDeviceToHost)) ;
...
)
{
	...
	checkCuda(cudaMemcpyAsync(A_gpu->acc_L_buff, &A_gpu->LnzvalVec[A_gpu->LnzvalPtr_host[kljb]],
				  l_copy_len * sizeof(float), cudaMemcpyDeviceToHost, CopyStream ) );

    if (copyU_kljb)
	checkCuda(cudaMemcpyAsync(A_gpu->acc_U_buff, &A_gpu->UnzvalVec[A_gpu->UnzvalPtr_host[kijb]],
				  u_copy_len * sizeof(float), cudaMemcpyDeviceToHost, CopyStream ) );
...
)
{
	...
	checkCudaErrors(cudaMallocHost(  &tmp_ptr,  remain_l_max * sizeof(float) )) ;
	A_gpu->scubufs[streamId].Remain_L_buff_host = (float *) tmp_ptr;
	checkCudaErrors(cudaMallocHost(  &tmp_ptr,  bigu_size * sizeof(float) )) ;
	A_gpu->scubufs[streamId].bigU_host = (float *) tmp_ptr;

	checkCudaErrors(cudaMallocHost ( &tmp_ptr, sizeof(float) * (A_host->bufmax[1])));
	A_gpu->acc_L_buff = (float *) tmp_ptr;
	checkCudaErrors(cudaMallocHost ( &tmp_ptr, sizeof(float) * (A_host->bufmax[3])));
	...
	checkCudaErrors(cudaMalloc(  &tmp_ptr,  remain_l_max * sizeof(float) )) ;
	A_gpu->scubufs[streamId].Remain_L_buff = (float *) tmp_ptr;
	gpu_mem_used += remain_l_max * sizeof(float);
	checkCudaErrors(cudaMalloc(  &tmp_ptr,  bigu_size * sizeof(float) )) ;
	...
	checkCudaErrors(cudaMalloc(  &tmp_ptr,  buffer_size * sizeof(float))) ;
	...
	checkCudaErrors(cudaMalloc(  &tmp_ptr,  l_val_len * sizeof(float)));
	A_gpu->LnzvalVec = (float *) tmp_ptr;
	checkCudaErrors(cudaMemset( (A_gpu->LnzvalVec), 0, l_val_len * sizeof(float)));
	...
	checkCudaErrors(cudaMalloc(  &tmp_ptr,  u_val_len * sizeof(float)));
	A_gpu->UnzvalVec = (float *) tmp_ptr;
	checkCudaErrors(cudaMemset( (A_gpu->UnzvalVec), 0, u_val_len * sizeof(float)));
	...
	checkCudaErrors(cudaMalloc( &tmp_ptr,  sizeof(sLUstruct_gpu_t))) ;
	*dA_gpu = (sLUstruct_gpu_t *) tmp_ptr;
	checkCudaErrors(cudaMemcpy( *dA_gpu, A_gpu, sizeof(sLUstruct_gpu_t), cudaMemcpyHostToDevice)) ;
