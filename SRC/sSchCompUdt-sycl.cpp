...
#pragma omp for schedule( SCHEDULE_STRATEGY )
		...
		// infrastructure for cudaMemcpy2DAsync functionality in SYCL
		void* to_ptr = dA;
		void* from_ptr = &lusup[luptr+(knsupc-ldu)*nsupr];
		size_t to_pitch = nbrow*sizeof(float);
		size_t from_pitch = nsupr*sizeof(float);
		size_t x = nbrow*sizeof(float);
		size_t y = ldu;
		...
		for (size_t z = 0; z < size.get(2); ++z) {
		  ...
		  for (size_t y = 0; y < size.get(1); ++y) {
		    streams[0].memcpy(to_ptr, from_ptr, size.get(0));
		...
		if ( num_col_stream > 0 ) {		
		    ...
		    streams[stream_id].memcpy(dB+b_offset, tempu+b_offset, B_stream_size); //memcpyAsync
