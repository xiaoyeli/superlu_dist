...
#include "slustruct_gpu_sycl.hpp"
...
void Scatter_GPU_kernel(
    ...
    sLUstruct_gpu_t * A_gpu, sycl::nd_item<3>& item,
    ...
    localAcc ljb_ind, localAcc lib_ind) /* local */
{
	...
	int iuip_lib =
	    local_u_blk_infoVec[local_u_blk_infoPtr[lib] + *ljb_ind]
	    .iuip;
	int ruip_lib =
	    local_u_blk_infoVec[local_u_blk_infoPtr[lib] + *ljb_ind]
	    .ruip;
...
// static void printDevProp(dpct::device_info devProp)
// {
// 	size_t mfree, mtotal;
...
int sfree_LUstruct_gpu (sLUstruct_gpu_t * A_gpu, ssluGPU_t *sluGPU)
{
...
//                        .memcpy(cPtr, dA, lda * n * sizeof(float))
//                        .wait(),
//                    0));
...
)
{
    ...
    if (grid3d->iam == 0)
    {
	printf("sinitSluGPU3D_t: Using hardware acceleration, with %d cuda streams, max_buffer_size %d\n", sluGPU->nCudaStreams, (int) buffer_size);
	fflush(stdout);
...
)
{
	...
	A_gpu->scubufs[streamId].Ublock_info_host = (Ublock_info_t*)tmp_ptr;
        tmp_ptr = (void *)sycl::malloc_host(remain_l_max * sizeof(float), *q);
        A_gpu->scubufs[streamId].Remain_L_buff_host = (float *) tmp_ptr;
	tmp_ptr = (void *)sycl::malloc_host(bigu_size * sizeof(float), *q);
        A_gpu->scubufs[streamId].bigU_host = (float *) tmp_ptr;

	tmp_ptr = (void *)sycl::malloc_host(sizeof(float) * (A_host->bufmax[1]), *q);
	A_gpu->acc_L_buff = (float *) tmp_ptr;
	tmp_ptr = (void *)sycl::malloc_host(sizeof(float) * (A_host->bufmax[3]), *q);
	...
	A_gpu->scubufs[streamId].usub_buf = (int_t *) tmp_ptr;
        ...
        tmp_ptr = (void *)sycl::malloc_device(remain_l_max * sizeof(float), *q);
        A_gpu->scubufs[streamId].Remain_L_buff = (float *) tmp_ptr;
	gpu_mem_used += remain_l_max * sizeof(float);
        tmp_ptr = (void *)sycl::malloc_device(bigu_size * sizeof(float), *q);
	...
	gpu_mem_used += mrb * sizeof(Remain_info_t);
        tmp_ptr = (void *)sycl::malloc_device(buffer_size * sizeof(float), *q);
	...
	q->memcpy( (A_gpu->LrowindVec), indtemp, l_ind_len * sizeof(int_t) );
        ...
        tmp_ptr = (void *)sycl::malloc_device(l_val_len * sizeof(float), *q);
        A_gpu->LnzvalVec = (float *) tmp_ptr;
	q->memset( (A_gpu->LnzvalVec), 0, l_val_len * sizeof(float));
	...
	q->memcpy( (A_gpu->UrowindVec), indtemp1, u_ind_len * sizeof(int_t) );
        ...
        tmp_ptr = (void *)sycl::malloc_device(u_val_len * sizeof(float), *q);
        A_gpu->UnzvalVec = (float *) tmp_ptr;
	q->memset( (A_gpu->UnzvalVec), 0, u_val_len * sizeof(float) );
	...
	q->memcpy( (A_gpu->xsup), xsup, (nsupers + 1)*sizeof(int_t) );
        ...
        tmp_ptr = (void *)sycl::malloc_device(sizeof(sLUstruct_gpu_t), *q);
	*dA_gpu = (sLUstruct_gpu_t *) tmp_ptr;
	q->memcpy( *dA_gpu, A_gpu, sizeof(sLUstruct_gpu_t) );
