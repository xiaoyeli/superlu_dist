...
#pragma omp for schedule(SCHEDULE_STRATEGY)
      ...
      if (num_streams_used > 0) {
        ...
        gpuMemcpy2DAsync(dA, nbrow * sizeof(float),
                         ...
                         nsupr * sizeof(float), nbrow * sizeof(float), ldu,
      ...
      for (int i = 0; i < num_streams_used; ++i) { // streams on GPU
        ...
        if (num_col_stream > 0) {
          ...
          gpublasCheckErrors(gpublasSgemm(handle[stream_id], GPUBLAS_OP_N,
          ...
          my_sgemm_("N", "N", &nbrow, &num_col_stream, &ldu, &alpha,
...
#pragma omp for schedule(SCHEDULE_STRATEGY) nowait
            ...
            for (lb = 0; lb < nlb; lb++) {
              ...
              if (ib < jb) { /* A(i,j) is in U. */
                ...
                sscatter_u(ib, jb, nsupc, iukp, xsup, klst, nbrow, lptr,
                ...
                sscatter_l(ib, ljb, nsupc, iukp, xsup, klst, nbrow, lptr,
