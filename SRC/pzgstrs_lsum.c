 ...
 * </pre>
 */
void zlsum_fmod_inv_master_onesided
/************************************************************************/
(
 doublecomplex *lsum,    /* Sum of local modifications.                        */
 doublecomplex *x,       /* X array (local)                                    */
			...
			if ( fmod_tmp==0 ) { /* Local accumulation done. */
				...
				if ( iam != p ) {
					...
					// #endif
					{
						...
						zlsum_fmod_inv_master_onesided(lsum, x, &x[ii], rtemp, nrhs, iknsupc, ik,
								fmod, nlb1, xsup,
								grid, Llu, stat,sizelsum,sizertemp,1+recurlevel,maxsuper,thread_id,num_thread,
...
} /* zlsum_bmod_inv_master_onesided */


/************************************************************************/
void zlsum_bmod_inv_master_onesided
/************************************************************************/
(
 doublecomplex *lsum,        /* Sum of local modifications.                    */
 doublecomplex *x,           /* X array (local).                               */
			...
			if ( iam != p ) {
				...
				// if ( !brecv[ik] ) { /* Becomes a leaf node. */
					...
					if ( Urbs[lk1] ){
						...
						// #endif
						{
						zlsum_bmod_inv_master_onesided(lsum, x, &x[ii], rtemp, nrhs, gik, bmod, Urbs,
								Ucb_indptr, Ucb_valptr, xsup, grid, Llu,
								stat, sizelsum,sizertemp,thread_id,num_thread,
                                iam_row, RDcount, RDbase, iam_col, BCcount, BCbase, Pc, maxrecvsz);
						}
