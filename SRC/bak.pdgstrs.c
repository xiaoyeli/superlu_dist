	...
	SuperLUStat_t *stat, int *info)
{
    ...
    double *recvbuf, *recvbuf_on, *tempv,
            *recvbufall, *recvbuf_BC_fwd, *recvbuf_RD_fwd, *recvbuf0, *xin;
...
#pragma omp simd
	...
	memset(recv_size_all, 0, (Pr+Pc) * sizeof(int));
        ...
        for (i=0; i<Pr;i++){
                if (iam_col!=i){ 
                        MPI_Irecv(&recv_size_all[i], 1, MPI_INT, i, 0, col_comm, &my_req);
        	        MPI_Isend(&nfrecvx,1, MPI_INT, i, 0, col_comm, &my_req);
                }
        }        
        recv_size_all[iam_col]=nfrecvx; 
        ...
        for (i=0; i<Pc;i++){
                if (iam_row!=i){ 
                        MPI_Irecv(&recv_size_all[Pr+i],  1, MPI_INT, i, 0, row_comm, &my_req);
        	        MPI_Isend(&nfrecvmod,1, MPI_INT, i, 0, row_comm, &my_req);
        
                }
        }        
        recv_size_all[Pr+iam_row]=nfrecvmod; 
...
//        printf("iam %d, BClocal_buf_id=%d,RDlocal_buf_id=%d\n",iam,BClocal_buf_id,RDlocal_buf_id);
	...
	BC_buffer_size=Pr*nfrecvx*maxrecvsz;
	RD_buffer_size=Pc*nfrecvmod*maxrecvsz;
...
while( nfrecv1 <= nfrecvx+nfrecvmod ){
	...
	BC_subtotal_all=0;
        RD_subtotal_all=0;
        for (i=0;i<Pr;i++) {
                BC_subtotal[i] = BC_taskq[i];	
                //nfrecv += BCis_solved[i];	
                BC_subtotal_all += BC_subtotal[i] - BCis_solved[i];
                //printf("2--------rank %d---BC_subtotal[%d]=%d, BC_taskq[%d]=%d,BCis_solved[%d]=%d\n",iam,i,BC_subtotal[i],i,(int)BC_taskq[i],i,BCis_solved[i]);      
                //fflush(stdout);
        }        
                
        for (i=0;i<Pc;i++) {
                //j=i-Pr;
                RD_subtotal[i] = RD_taskq[i];
                //nfrecv += RDis_solved[j];
                RD_subtotal_all += RD_subtotal[i] - RDis_solved[i];
                //printf("3--------rank %d---RD_subtotal[%d]=%lf, RD_taskq[%d]=%lf,RDis_solved[%d]=%lf\n",iam,i,RD_subtotal[i],i,RD_taskq[i],i,RDis_solved[i]);      
                //fflush(stdout);
        }
	
        nfrecv1 += RD_subtotal_all + BC_subtotal_all;
        if (BC_subtotal_all+RD_subtotal_all==0) continue;
        //printf("Iam %d--debug_count=%d,nfrecv=%lf,BC_subtotal_all=%lf,RD_subtotal_all=%lf,nfrecvx=%d,nfrecvmod=%d\n",iam,debug_count,nfrecv1,BC_subtotal_all,RD_subtotal_all,nfrecvx,nfrecvmod);      
        //fflush(stdout);
	...
	if (BC_subtotal_all > 0){
		...
		for (bcidx=0;bcidx<Pr;bcidx++){
			...
			if ( BC_subtotal[bcidx] > BCis_solved[bcidx]){
                              //  printf("6--------rank %d---BC_taskq[%d]=%lf,BCis_solved[%d]=%d---\n",iam,bcidx,BC_taskq[bcidx],bcidx,BCis_solved[bcidx]);      
                              //  fflush(stdout);


                                BC_taskbuf_offset = Pr + bcidx*(nfrecvx)*maxrecvsz;
                    //                     printf("6.1 -- rank %d, j=%d,recv_size_all[%d]=%d\n",iam,j,j,recv_size_all[j]);
	     			
                   //             printf("7--------rank %d, bcdix=%d,BC_taskbuf_offset=%d\n",iam,bcidx, BC_taskbuf_offset);      
                   //             fflush(stdout);
        ...
        if (RD_subtotal_all > 0 ){
                for (rdidx=0;rdidx<Pc;rdidx++){
                        if (rdidx == iam_row) continue;
                        if ( RD_subtotal[rdidx] > RDis_solved[rdidx]){
...
#pragma omp simd
                                ...
                                //printf("33--------rank %d,RDis_solved[%d]=%d,totaltaskthisround=%d\n",iam,rdidx,RDis_solved[rdidx],nfrecv);      
                               // fflush(stdout);
			} // loop rd bufferk  
		} // if have rd task
		}// loop rd buffer
      ...
      //int tmp_task=0, tmp_task1=0;
      //int tmp_solve=0, tmp_solve1=0;
      //for (int kkkkk=0; kkkkk<Pr;kkkkk++){
      //       tmp_task += (int) BC_taskq[kkkkk];
      //       tmp_solve += BCis_solved[kkkkk]; 
      //}
      //for (int kkkkk=0; kkkkk<Pc;kkkkk++){
      //       tmp_task1 += (int) RD_taskq[kkkkk];
      //       tmp_solve1 += RDis_solved[kkkkk]; 
      //}
      //printf("14----rank %d---BCis_solved=%d--BCtaskq=%d/%d--RDis_solved=%d---RDtaskq=%d/%d\n",iam,tmp_solve,tmp_task,nfrecvx,tmp_solve1,tmp_task1,nfrecvmod);      
      //fflush(stdout);
}// outer-most while 
}} // two omp pragma
//printf("I am %d, iam out\n",iam);
//fflush(stdout);
//MPI_Barrier(MPI_COMM_WORLD);
		...
		if ( !iam ) {
			printf(".. L-solve time\t%f, DGEM time\t %f, COMM time\t %f\n", t, stat_loc[0]->utime[SOL_TRSM]+stat_loc[0]->utime[SOL_GEMM],stat_loc[0]->utime[SOL_COMM]);
			fflush(stdout);
		}
...
#ifdef oneside
        MPI_Group_free(&row_comm);
        MPI_Group_free(&col_comm);
