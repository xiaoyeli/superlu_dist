...
//          if(this->tag_==9){gdb_lock();}
		...
		// changed by Yang Liu: if not ready_, no active message, but expecting incoming message, then post recv 	
		if(this->recvCount_==this->recvPostedCount_){
			if(this->recvPostedCount_<this->GetDestCount()){
				this->postRecv();
				retVal = false;
			}		  
		}
  ...
  template< typename T> 
    inline bool TreeReduce_v2<T>::Progress(){
      ...
      bool retVal = false, tmp;
	  
		  // if(this->tag_==15 && this->IsRoot()){
			 // std::cout<<this->GetDestCount()<<" "<<this->recvPostedCount_<<"YESYES"<<std::endl; 
		  // }	  
      ...
      if(this->done_){
        ...
        if(this->isAllocated_){
		  ...
		  tmp = this->IsDataReceived();  // changed by Yang Liu, test receiving even not isReady_
