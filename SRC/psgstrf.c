 ...
 * </pre>
 */
int_t psgstrf(superlu_dist_options_t *options, int m, int n, float anorm,
  ...
  *info = 0;
  ...
  if (Pr * Pc > 1) {
    ...
    if (i != 0) {
      if (!(Llu->Lval_buf_2[0] =
                floatMalloc_dist((num_look_aheads + 1) * ((size_t)i))))
  ...
  if (!(Lval_buf_2 =
            (float **)SUPERLU_MALLOC((1 + num_look_aheads) * sizeof(float *))))
...
#pragma omp master
  ...
  sstatic_schedule(options, m, n, LUstruct, grid, stat, perm_c_supno,
                   iperm_c_supno, info);
...
#pragma omp parallel for reduction(max : local_max_row_size) private(lk, lsub)
  ...
  int_t buffer_size = SUPERLU_MAX(max_row_size * max_ncols, sp_ienv_dist(8, options));
  //   get_max_buffer_size());
  ...
  if (superlu_acc_offload) {
    ...
    /* if (checkGPU(gpuMallocHost(&bigU, bigu_size * sizeof(float)))) */
    ...
    /* if (checkGPU(gpuMallocHost((void **)&bigV, bigv_size * sizeof(float)))) */
  ...
  //     bigU = _mm_malloc(bigu_size * sizeof(float), 1<<12); // align at 4K
  //     page bigV = _mm_malloc(bigv_size * sizeof(float), 1<<12);
  // #else
  if (!(bigU = floatMalloc_dist(bigu_size)))
  ...
  if (mycol == kcol) {
    ...
    for (pj = 0; pj < Pc; ++pj) {
      if (ToSendR[lk][pj] != EMPTY) {
        ...
        MPI_Isend(lusup, msgcnt[1], MPI_FLOAT, pj, SLU_MPI_TAG(1, 0) /* 1 */,
        ...
        if (mycol == kcol) { /* I own this panel */
          ...
          if (ToRecv[kk] >= 1) {
            ...
            MPI_Irecv(Lsub_buf_2[look_id], Llu->bufmax[0], mpi_int_t, kcol,
                      SLU_MPI_TAG(0, kk0), /* (4*kk0)%tag_ub */
                      scp->comm, &recv_req[0]);
            MPI_Irecv(Lval_buf_2[look_id], Llu->bufmax[1], MPI_FLOAT, kcol,
                      SLU_MPI_TAG(1, kk0), /* (4*kk0+1)%tag_ub */
                      scp->comm, &recv_req[1]);
#if (PROFlevel >= 1)
            TOC(t2, t1);
...
#endif
            {
              psgstrs2_omp(kk0, kk, Glu_persist, grid, Llu, Ublock_info, stat);
            }
            ...
            if (ToSendD[lk] == YES) {
              for (pi = 0; pi < Pr; ++pi) {
                if (pi != myrow) {
                  ...
                  MPI_Isend(uval, msgcnt[3], MPI_FLOAT, pi,
...
/* #pragma omp parallel */ /* Sherry -- parallel done inside psgstrs2 */
#endif
        { psgstrs2_omp(k0, k, Glu_persist, grid, Llu, Ublock_info, stat); }
        pdgstrs2_timer += SuperLU_timer_() - ttt2;
