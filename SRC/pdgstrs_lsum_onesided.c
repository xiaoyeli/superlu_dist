...
#ifdef one_sided
void dlsum_fmod_inv_master_onesided
(
...
// #endif
                    {
                        nlb1 = lsub1[0] - 1;
#if ( DEBUGlevel>=2 )
                    printf("iam=%d, lk=%d, in master enter master\n",iam, lk);
                    fflush(stdout);
#endif
                        dlsum_fmod_inv_master_onesided(lsum, x, &x[ii], rtemp, nrhs, iknsupc, ik,
                                              fmod, nlb1, xsup,
                                              grid, Llu, stat,sizelsum,sizertemp,1+recurlevel,maxsuper,thread_id,num_thread,
                                              RDcount, RDbase, BCcount, BCbase, Pc, maxrecvsz);
#if ( DEBUGlevel>=2 )
                    printf("iam=%d, lk=%d, in master out master\n",iam, lk);
                    fflush(stdout);
#endif
                    }
...
/************************************************************************/
void dlsum_bmod_inv_master_onesided
/************************************************************************/
(
 double *lsum,        /* Sum of local modifications.                    */
 double *x,           /* X array (local).                               */
...
// #endif
						{
						dlsum_bmod_inv_master_onesided(lsum, x, &x[ii], rtemp, nrhs, gik, bmod, Urbs,
