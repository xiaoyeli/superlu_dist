...
#pragma omp for schedule( SCHEDULE_STRATEGY )
		...
		size_t to_pitch = nbrow*sizeof(double);
		size_t from_pitch = nsupr*sizeof(double);
		size_t x = nbrow*sizeof(double);
		size_t y = ldu;
		...
		if ( num_col_stream > 0 ) {
		    ...
		    streams[stream_id].memcpyAsync(dB+b_offset, tempu+b_offset, B_stream_size);
