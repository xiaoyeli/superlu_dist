...
int_t lk,k,knsupc;
	...
	if(nrhs>1){
		...
		dlsum_fmod_inv_cuda_mrhs<<< nblock, dimBlock >>>(lsum,x,rtemp,nrhs,maxsup,nsupers,fmod,xsup,grid,Llu,order);
		// dlsum_fmod_inv_cuda<<< 4, 32 >>>(lsum,x,rtemp,nrhs,maxsup,nsupers,fmod,xsup,grid,Llu);	
	}else{
		dim3 dimBlock(nthread_x*nthread_y, 1);
		dlsum_fmod_inv_cuda_1rhs<<< CEILING(nblock,NWARP), dimBlock >>>(lsum,x,rtemp,nrhs,maxsup,nsupers,fmod,xsup,grid,Llu,order);
		// dlsum_fmod_inv_cuda<<< 4, 32 >>>(lsum,x,rtemp,nrhs,maxsup,nsupers,fmod,xsup,grid,Llu);	
	}
