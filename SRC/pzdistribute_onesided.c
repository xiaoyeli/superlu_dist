...
float
pzdistribute_onesided(superlu_dist_options_t *options, int_t n, SuperMatrix *A,
...
//#endif
    ...
    //printf("Check who is here: iam=%d\n",iam);
    //fflush(stdout);
    int color = iam % Pc;
    MPI_Comm_split(grid->comm, color, iam, &col_comm);

    //printf("Check MPI_Comm_split, iam=%d\n",iam);
    //fflush(stdout);
    int iam_col;
    MPI_Comm_rank(col_comm, &iam_col);

    color = iam / Pc;
    MPI_Comm_split(grid->comm, color, iam, &row_comm);
    int iam_row;
    MPI_Comm_rank(row_comm, &iam_row);
    //printf("iam=%d,iam_col=%d,iam_row=%d\n",iam,iam_col,iam_row);
    //fflush(stdout);
	...
	if ( !(Unzval_br_offset =(long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
    ...
    if ( !(Ufstnz_br_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
    ...
    if ( !(Lrowind_bc_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
        fprintf(stderr, "Malloc fails for Lrowind_bc_offset[].");
    }
    Lrowind_bc_offset[k-1] = -1;
    if ( !(Lnzval_bc_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
    ...
    if ( !(Lindval_loc_bc_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
    ...
    if ( !(Linv_bc_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
    ...
    if ( !(Uinv_bc_offset = (long int*)SUPERLU_MALLOC(k * sizeof(long int))) ) {
	...
	for (ljb = 0; ljb < k; ++ljb) { /* for each local block column ... */
		...
		if(Iactive==1){
			...
			if(rank_cnt>1){
                ...
                C_BcTree_Create_onesided(&LBtree_ptr[ljb], grid->comm, ranks, rank_cnt, msgsize, 'z', BufSize, Pc);
		...
		if(ib<nsupers){
			...
			if(Iactive==1){
				...
				if(rank_cnt>1){
					...
					C_RdTree_Create_onesided(&LRtree_ptr[lib], grid->comm, ranks, rank_cnt, msgsize, 'z', BufSize_rd,Pc);
...
if ( !iam) printf(".. Construct Reduce tree for L: %.2f\t\n", t);
	...
	// fflush(stdout);
			if(rank_cnt>1){
				...
				C_BcTree_Create_onesided(&UBtree_ptr[ljb], grid->comm, ranks, rank_cnt, msgsize, 'z', BufSize_u, Pc);
		...
		if(ib<nsupers){
			...
			if(Iactive==1){
				...
				if(rank_cnt>1){
					...
					C_RdTree_Create_onesided(&URtree_ptr[lib], grid->comm, ranks, rank_cnt, msgsize, 'z', BufSize_urd, Pc);
...
if ( !iam) printf(".. Construct Reduce tree for U: %.2f\t\n", t);
    ...
    BC_taskq = (doublecomplex*)SUPERLU_MALLOC( BC_buffer_size * sizeof(doublecomplex));
...
#else
    MPI_Win_create(BC_taskq, (BC_buffer_size)*sizeof(doublecomplex), sizeof(doublecomplex), MPI_INFO_NULL, col_comm, &bc_winl);
#endif
    int nfrecvmod=0;
    for (lk=0;lk<CEILING( nsupers, grid->nprow );++lk){
        if(LRtree_ptr[lk].empty_==NO){
            nfrecvmod += LRtree_ptr[lk].destCnt_;
        }
    }
    int nbrecvmod=0 ;
	for (lk=0;lk<CEILING( nsupers, grid->nprow );++lk){
...
#else
	MPI_Win_create(RD_taskq, (RD_buffer_size)*sizeof(doublecomplex), sizeof(doublecomplex), MPI_INFO_NULL, row_comm, &rd_winl);
#endif

#if ( DEBUGlevel>=1 )
    printf("iam=%d, maxrecvsz=%d, maxBCmsg=%d (%f MB),maxRDmsg=%d (%f MB)\n",
            iam, maxrecvsz,BC_buffer_size/maxrecvsz,((BC_buffer_size)*sizeof(doublecomplex))/1e6, RD_buffer_size/maxrecvsz,((RD_buffer_size)*sizeof(doublecomplex))/1e6);
    fflush(stdout);
#endif
