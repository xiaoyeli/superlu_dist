...
export LD_LIBRARY_PATH=/opt/rocm-6.2.4/lib:$(mpicc --showme:libdirs 2>/dev/null | tr ' \n' '::' | sed 's/:*$//'):$LD_LIBRARY_PATH
# module load cudatoolkit/11.7
# avoid bug in cray-libsci/21.08.1.2
# module load cray-libsci/22.11.1.2
# module load cray-libsci/23.02.1.1
ulimit -s unlimited
# Force OpenBLAS to use AVX2 (Zen3) kernels instead of AVX-512 (skylakex).
# The installed OpenBLAS was compiled for Intel skylakex; EPYC 7763 only has AVX2.
...
MATPREFIX=/home/users/tshi777/ECP_Tutorial_GPU_Capable_Sparse_Direct_Solvers/instinct/manual_install/matrix/EMTrun1/EMTrun1_

echo "================ SINGLE psdrive3d_vbatch, no IR (-i 0) ================"
mpirun -np 1 ./EXAMPLE/psdrive3d_vbatch -b 2 -i 0 -f .dat $MATPREFIX

echo "================ SINGLE psdrive3d_vbatch, double-precision IR (-i 2) ================"
mpirun -np 1 ./EXAMPLE/psdrive3d_vbatch -b 2 -i 2 -f .dat $MATPREFIX
