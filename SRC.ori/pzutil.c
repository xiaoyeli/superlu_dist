...
void
zDestroy_Tree(int_t n, gridinfo_t *grid, LUstruct_t *LUstruct)
{
    int_t i, nb, nsupers;
	...
	for (i=0;i<nb;++i){
		if(Llu->LBtree_ptr[i]!=NULL){
			BcTree_Destroy(Llu->LBtree_ptr[i],'z');
		}
		if(Llu->UBtree_ptr[i]!=NULL){
			BcTree_Destroy(Llu->UBtree_ptr[i],'z');
		}		
	}
	...
	for (i=0;i<nb;++i){
		if(Llu->LRtree_ptr[i]!=NULL){
			RdTree_Destroy(Llu->LRtree_ptr[i],'z');
		}
		if(Llu->URtree_ptr[i]!=NULL){
			RdTree_Destroy(Llu->URtree_ptr[i],'z');
		}		
	}
