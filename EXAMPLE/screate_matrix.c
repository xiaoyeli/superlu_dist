 ...
 * =======
 * 
 * SCREATE_MATRIX_POSTFIX read the matrix from data file in different formats
                   ...
                   FILE *fp, char * postfix, gridinfo_t *grid)
{
    ...
    *trans = 'N';
    ...
    if (iam == 0) {
        ...
        MPI_Bcast( xtrue_global, n*nrhs, MPI_FLOAT, 0, grid->comm );
        MPI_Bcast( b_global, m*nrhs, MPI_FLOAT, 0, grid->comm );
    } else {
        MPI_Bcast( xtrue_global, n*nrhs, MPI_FLOAT, 0, grid->comm );
        MPI_Bcast( b_global, m*nrhs, MPI_FLOAT, 0, grid->comm );
    }
