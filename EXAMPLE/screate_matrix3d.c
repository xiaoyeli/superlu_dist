 ...
 * =======
 *
 * SCREATE_MATRIX3D_POSTFIX read the matrix from data file with the
 ...
 * =======
 *
 * SCREATE_BLOCK_DIAG_3D read the matrix from data file with the
 * storage format described by the suffix string "postfix";
 ...
 * </pre>
 */
int screate_block_diag_3d(SuperMatrix *A, int batchCount, int nrhs, float **rhs,
                           int *ldb, float **x, int *ldx,
                           FILE *fp, char * postfix, gridinfo3d_t *grid3d)
{
    SuperMatrix GA;              /* global A */
 ...
 */
int screate_batch_systems_multiple(handle_t *SparseMatrix_handles, int batchCount,
                 int nrhs, float **RHSptr, int *ldRHS, float **xtrue, int *ldX,
                 FILE **fp, char * postfix, gridinfo3d_t *grid3d)
{
    int_t    *rowind_d, *colptr_d;  /* metadata for one diagonal block */
    float   *nzval_d;      /* global */
    int_t    m, n, nnz;
    int    row, col, i, j, relpos;
    int      iam;
    char     trans[1];
...
CHECK_MALLOC(iam, "Enter dcreate_batch_systems()");
    ...
    for (d = 0; d < batchCount; ++d) {
        ...
        if ( !iam )
        {
            ...
            {
                /* Read the matrix stored on disk in Harwell-Boeing format. */
                sreadhb_dist(iam, fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "mtx"))
            {
                /* Read the matrix stored on disk in Matrix Market format. */
                sreadMM_dist(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "rb"))
            {
                /* Read the matrix stored on disk in Rutherford-Boeing format. */
                sreadrb_dist(iam, fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "dat"))
            {
                /* Read the matrix stored on disk in triplet format. */
                sreadtriple_dist(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "datnh"))
            {
                /* Read the matrix stored on disk in triplet format (without header). */
                sreadtriple_noheader(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "bin"))
            {
                /* Read the matrix stored on disk in binary format. */
                sread_binary(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else
            {
                ABORT("File format not known");
...
#endif
    return 0;
} /* end screate_batch_systems_multiple */
 ...
 * with sbatch_systems_free() before reading the next time step.
 */
int screate_batch_systems_rhsfile(handle_t *SparseMatrix_handles, int batchCount,
                 int nrhs, float **RHSptr, int *ldRHS, float **xtrue, int *ldX,
                 ...
                 gridinfo3d_t *grid3d)
{
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter screate_batch_systems_rhsfile()");
#endif
    ...
    for (d = 0; d < batchCount; ++d) {
        ...
        if (!strcmp(postfix, "rua"))
            sreadhb_dist(iam, fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if ( (!strcmp(postfix, "mtx")) || (!strcmp(postfix, "mm")) )
            sreadMM_dist(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "rb"))
            sreadrb_dist(iam, fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "dat"))
            sreadtriple_dist(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "datnh"))
            sreadtriple_noheader(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "bin"))
            sread_binary(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else
            ABORT("File format not known");
        ...
        /* The readers above hand back storage they allocated; hand it straight
           ...
           screate_batch_systems* routines do. */
        SuperMatrix *Ad = (SuperMatrix *) SUPERLU_MALLOC( sizeof(SuperMatrix) );
        sCreate_CompCol_Matrix_dist(Ad, m, n, nnz, nzval, rowind, colptr,
                                    SLU_NC, SLU_S, SLU_GE);
        ...
        for (j = 0; j < nrhs; ++j)
            for (i = 0; i < m; ++i)
                if ( fscanf(fpB[d], "%f", &RHSptr[d][j*m + i]) != 1 )
        ...
        if ( fpX ) {
            ...
            for (j = 0; j < nrhs; ++j)
                for (i = 0; i < n; ++i)
                    if ( fscanf(fpX[d], "%f", &xtrue[d][j*n + i]) != 1 )
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Exit screate_batch_systems_rhsfile()");
#endif
    return 0;
...
void sbatch_systems_free(handle_t *SparseMatrix_handles, int batchCount,
