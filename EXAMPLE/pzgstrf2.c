...
pzgstrf2_trsm
    (superlu_options_t * options, int_t nsupers,
     ...
     int tag_ub, SuperLUStat_t * stat, int *info)
{
    ...
    doublecomplex one = {1.0, 0.0}, alpha = {-1.0, 0.0}, zero = {0.0, 0.0};
    int_t Pr;
    MPI_Status status;
    ...
    *info = 0;
    ...
    if (iam == pkk) {            /* diagonal process */
        for (j = 0; j < jlst - jfst; ++j) {  /* for each column in panel */
           ...
           if ( options->ReplaceTinyPivot == YES ) {
            ...
            for (l = 0; l < cols_left; ++l, i += nsupr, ++u_diag_cnt) {
                st = j * ld_ujrow + j;
                ublk_ptr[st + l * ld_ujrow].r = lusup[i].r; /* copy one row of U */
                ublk_ptr[st + l * ld_ujrow].i = lusup[i].i;
            }

#if 0
	    if (k<=4) printf("[%d] k = %d, j = %d, nsupr=%d, DIAG: (%e, %e)\n",
			     iam, k, j, nsupr, ujrow[0].r, ujrow[0].i);
#endif
		...
		if (k==4)  PrintDoublecomplex("before scale:", nsupr-j-1, &lusup[luptr+1]);
		...
		if (k==4) PrintDoublecomplex("after scale:", nsupr-j-1, &lusup[luptr+1]);
	...
	if ( k<=4 ) {
	    printf("[%d] k = %d, nsupr=%d, DIAG: (%e, %e)\n",
		   iam, k, nsupr, lusup[0].r, lusup[0].i);
	    printf("iam [%d]\n", iam);
	    /*PrintDoublecomplex("after ztrsm:", l, &lusup[nsupc]);*/
	    PrintDoublecomplex("after ztrsm:", nsupr, lusup);
	}
...
void pzgstrs2_omp
/************************************************************************/
(int_t m, int_t k0, int_t k, Glu_persist_t * Glu_persist,
