/*! @file
 * \brief Perform LU factorization in parallel
 *
 * <pre> 
...
/*#pragma _CRI cache_bypass nzval,tempv*/
	...
	if ( msg0 && msg2 ) { /* L(:,k) and U(k,:) are not empty. */
		...
		for (lb = 0; lb < nlb; ++lb) { 
			...
			for (jj = 0; jj < nsupc; ++jj) {
			    segsize = klst - usub[iukp + jj];
			    fnz = index[iuip[lib] + jj];
			    if ( segsize != 0 ) {
			       /* Index of a nonzero segment */
			        indirect[it] = jj;
				/* Location of the destination U segment */
				indirect[ldt + it] = ruip[lib];
				++it;
			    }
			    ruip[lib] += ilst - fnz;
			}
			...
			/* Can use IVDEP for this loop. */
			for (it = 0; it < nonzero_segs; ++it) {
			    jj = indirect[it];
			    fnz = index[iuip[lib] + jj];
			    ucol = &Unzval_br_ptr[lib][indirect[ldt + it]];
			    for (i = 0; i < nbrow; ++i) {
			        rel = lsub[lptr + i] - fnz;
				ucol[rel] -= tempv2d[i + it*ldt];
			    }
			}
			iuip[lib] += nsupc;
#else
			...
			for (jj = 0; jj < nsupc; ++jj) {
			    segsize = klst - usub[iukp + jj];
			    if ( segsize != 0 ) {
			      /* Index of a nonzero destination column */
			      indirect[ldt + it] = jj;
			      ++it;
			    }
			}
			...
			/* Can use IVDEP for this loop. */
			for (it = 0; it < nonzero_segs; ++it) {
			    jj = indirect[ldt + it]; /* Index of dest column */
			    nzval = tempv + jj * ldv;
			    for (i = 0; i < nbrow; ++i) {
			        rel = lsub[lptr + i] - fnz;
				nzval[indirect[rel]] -= tempv2d[i + it*ldt];
			    }
			}
#else
			tempv = tempv2d;
