 ...
 * =======
 *
 * ZCREATE_BLOCK_DIAG_3D read the matrix from data file with the
 * storage format described by the suffix string "postfix";
 ...
 * </pre>
 */
int zcreate_block_diag_3d(SuperMatrix *A, int batchCount, int nrhs, doublecomplex **rhs,
                           int *ldb, doublecomplex **x, int *ldx,
                           FILE *fp, char * postfix, gridinfo3d_t *grid3d)
{
    SuperMatrix GA;              /* global A */
 ...
 */
int zcreate_batch_systems_multiple(handle_t *SparseMatrix_handles, int batchCount,
                 int nrhs, doublecomplex **RHSptr, int *ldRHS, doublecomplex **xtrue, int *ldX,
                 FILE **fp, char * postfix, gridinfo3d_t *grid3d)
{
    int_t    *rowind_d, *colptr_d;  /* metadata for one diagonal block */
    doublecomplex   *nzval_d;      /* global */
    int_t    m, n, nnz;
    int    row, col, i, j, relpos;
    int      iam;
    char     trans[1];
...
CHECK_MALLOC(iam, "Enter dcreate_batch_systems()");
    ...
    for (d = 0; d < batchCount; ++d) {
        ...
        if ( !iam )
        {
            ...
            {
                /* Read the matrix stored on disk in Harwell-Boeing format. */
                zreadhb_dist(iam, fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "mtx"))
            {
                /* Read the matrix stored on disk in Matrix Market format. */
                zreadMM_dist(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "rb"))
            {
                /* Read the matrix stored on disk in Rutherford-Boeing format. */
                zreadrb_dist(iam, fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "dat"))
            {
                /* Read the matrix stored on disk in triplet format. */
                zreadtriple_dist(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "datnh"))
            {
                /* Read the matrix stored on disk in triplet format (without header). */
                zreadtriple_noheader(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else if (!strcmp(postfix, "bin"))
            {
                /* Read the matrix stored on disk in binary format. */
                zread_binary(fpd, &m, &n, &nnz, &nzval, &rowind, &colptr);
            }
            else
            {
                ABORT("File format not known");
...
#endif
    return 0;
} /* end zcreate_batch_systems_multiple */
 ...
 * with zbatch_systems_free() before reading the next time step.
 */
int zcreate_batch_systems_rhsfile(handle_t *SparseMatrix_handles, int batchCount,
                 int nrhs, doublecomplex **RHSptr, int *ldRHS, doublecomplex **xtrue, int *ldX,
                 ...
                 gridinfo3d_t *grid3d)
{
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Enter zcreate_batch_systems_rhsfile()");
#endif
    ...
    for (d = 0; d < batchCount; ++d) {
        ...
        if (!strcmp(postfix, "rua"))
            zreadhb_dist(iam, fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if ( (!strcmp(postfix, "mtx")) || (!strcmp(postfix, "mm")) )
            zreadMM_dist(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "rb"))
            zreadrb_dist(iam, fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "dat"))
            zreadtriple_dist(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "datnh"))
            zreadtriple_noheader(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else if (!strcmp(postfix, "bin"))
            zread_binary(fpA[d], &m, &n, &nnz, &nzval, &rowind, &colptr);
        else
            ABORT("File format not known");
        ...
        /* The readers above hand back storage they allocated; hand it straight
           ...
           zcreate_batch_systems* routines do. */
        SuperMatrix *Ad = (SuperMatrix *) SUPERLU_MALLOC( sizeof(SuperMatrix) );
        zCreate_CompCol_Matrix_dist(Ad, m, n, nnz, nzval, rowind, colptr,
                                    SLU_NC, SLU_Z, SLU_GE);
        ...
        for (j = 0; j < nrhs; ++j)
            for (i = 0; i < m; ++i)
                if ( fscanf(fpB[d], "%lf%lf", &RHSptr[d][j*m + i].r, &RHSptr[d][j*m + i].i ) != 1 )
        ...
        if ( fpX ) {
            ...
            for (j = 0; j < nrhs; ++j)
                for (i = 0; i < n; ++i)
                    if ( fscanf(fpX[d], "%lf%lf", &xtrue[d][j*n + i].r, &xtrue[d][j*n + i].i) != 1 )
                        ABORT("Cannot read the reference-solution values");
        } else {
	    doublecomplex zero = {0.0, 0.0};
            for (i = 0; i < n * nrhs; ++i) xtrue[d][i] = zero;
        }
    }

#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(iam, "Exit zcreate_batch_systems_rhsfile()");
#endif
    return 0;
...
void zbatch_systems_free(handle_t *SparseMatrix_handles, int batchCount,
