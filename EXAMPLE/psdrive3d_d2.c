...
/*! \brief The driver program PSDRIVE3D.
 *
 * <pre>
 * Purpose
 * =======
 *
 * This example illustrates how to use PSGSSVX3D or PSGSSVX3D_CSC_BATCH
...
int main (int argc, char *argv[])
{
	...
	/* Call batch solver */
	psgssvx3d_csc_batch(&options, batchCount,
...
#define NRFRMT
	...
	*/
    ...
    /* ---- double-precision iterative refinement (d2) setup ---- */
    float *err_bounds;        /* 3*nrhs forward/backward error bounds */
    double *xtrue_d;          /* true solution in double, for d2 diagnostics */
    if ( options.IterRefine <= SLU_SINGLE ) options.IterRefine = SLU_DOUBLE;
    if (!(err_bounds = floatMalloc_dist (3 * nrhs)))
    ...
    /* Provide the true solution to the d2 refinement in double precision
       (used only for PRNTlevel>=2 diagnostics; harmless otherwise). */
    {
        extern double *doubleMalloc_dist(int_t);
        if (!(xtrue_d = doubleMalloc_dist (ldx * nrhs)))
            ABORT ("Malloc fails for xtrue_d[].");
        for (int_t kk = 0; kk < ldx * nrhs; ++kk) xtrue_d[kk] = (double) xtrue[kk];
    }

    /* Call the linear equation solver with DOUBLE-precision iterative refinement. */
    psgssvx3d_d2 (&options, &A, &ScalePermstruct, b, ldb, nrhs, &grid,
                  &LUstruct, &SOLVEstruct, berr, err_bounds, &stat, &info, xtrue_d);

    if ( info ) {  /* Something is wrong */
        if ( iam==0 ) {
	    printf("ERROR: INFO = %d returned from psgssvx3d_d2()\n", info);
	    fflush(stdout);
	}
    } else {
        ...
        if ( iam == 0 ) {
            for (int jj = 0; jj < nrhs; ++jj)
                printf(".. d2 RHS %d: fwd-err-bound(norm) %e, fwd-err-bound(comp) %e, "
                       "Berr %e\n", jj, err_bounds[jj], err_bounds[jj+nrhs],
                       err_bounds[jj+2*nrhs]);
            fflush(stdout);
        }
    }
